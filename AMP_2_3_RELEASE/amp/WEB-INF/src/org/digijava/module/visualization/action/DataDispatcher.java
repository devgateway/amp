...
public class DataDispatcher extends DispatchAction {
	...
	public ActionForward applyFilter(ActionMapping mapping, ActionForm form,
		...
		try{
			...
			trnStep8 = TranslatorWorker.translateText("Step 10/10: Preparing to refresh charts", request);
		}
		catch(Exception e){
	...
	public ActionForward getProgramProfile(boolean NPO, ActionMapping mapping,
        ...
        try {
			...
			if(format != null && format.equals("xml")){
				...
				Iterator it = list.iterator();
				int index = 0;
	    		csvString.append("\""+TranslatorWorker.translateText("Program Name", locale, siteId)+"\"");
	    		csvString.append(",");
	    		csvString.append("\""+TranslatorWorker.translateText("Amount", locale, siteId)+"\"");
	    		csvString.append("\n");
	            BigDecimal othersValue = BigDecimal.ZERO;
	...
	public ActionForward getOrganizationProfileGraphData(ActionMapping mapping,
        ...
        try {
    				...
    				int index = 0;
    				 while(it.hasNext() && index <= 4){
     	               ...
     	               //if(percentage.compareTo(new BigDecimal(0.01)) == 1) //if this is more than 0.01
     	                if (donut){
     	                	if(percentage.compareTo(new BigDecimal(1)) == 1){
     	                		xmlString.append("<dataField name=\"" + org.getName().replace("\"", "") + "\" startYear=\"" + (startDate.getYear() + 1900) + "\" endYear=\"" + (endDate.getYear() + 1900) + "\" value=\""+ value.divide(divideByDenominator).setScale(filter.getDecimalsToShow(), RoundingMode.HALF_UP) + "\" label=\"" + org.getName().replace("\"", "") + "\" percentage=\"" + percentage.toPlainString() + "\" id=\"" + org.getAmpOrgId() + "\" yearLabels=\"" + yearLabels + "\"/>\n");
     	                		index++;
     	                	}
     	                } else {
     	                	if(percentage.compareTo(new BigDecimal(0.01)) == 1){
            ...
            else
            {
				...
				Iterator it = list.iterator();
				int index = 0;
        		csvString.append("\""+TranslatorWorker.translateText("Organization Name", locale, siteId)+"\"");
        		csvString.append(",");
        		csvString.append("\""+TranslatorWorker.translateText("Amount", locale, siteId)+"\"");
        		csvString.append("\n");
                BigDecimal othersValue = BigDecimal.ZERO;
	...
	public ActionForward getBudgetBreakdownGraphData(ActionMapping mapping,
       ...
       if(format != null && format.equals("xml")){
		...
		String title = TranslatorWorker.translateText("Unallocated",locale, siteId);
		csvString.append("\"");
		csvString.append(title);
		csvString.append("#");
        csvString.append("-1");
...
//			result = new DefaultCategoryDataset();
	...
	public ActionForward getAidPredictabilityQuarterGraphData(ActionMapping mapping,
        ...
        StringBuffer csvString = new StringBuffer();
        String text = TranslatorWorker.translateText("Period",locale, siteId);
