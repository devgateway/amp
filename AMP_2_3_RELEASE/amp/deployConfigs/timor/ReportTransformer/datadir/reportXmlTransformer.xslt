		...
		<script type="text/javascript" src="/TEMPLATE/ampTemplate/js_2/jquery/jquery-min.js"></script>
			...
			<body leftmargin="0" topmargin="0" rightmargin="0" bottommargin="0">
				<table border="0" class="table_text" width="100%" id="mainTable">
					<tr>
						<th>Donor Agency</th>
						<th colspan="2" align="center">Current Year</th>
				...
				<script type="text/javascript" language="javascript">
					...
					<xsl:for-each select="report/row">
						...
						<xsl:if test="position()>7">
							<xsl:for-each select="column">
								<xsl:if test="position()=1 or position()=32 or position()=33 or position()=59 or position()=61">
					...
					var colTooltips = new Array("Development Partner", "Planned Disb. (Current year)", "Actual Disb. (Current year)", "Committed (All Years)", "Disbursed (All Years)", "Disb. Rate");
					...
					for (rowIdx = 0; rowIdx <xsl:text disable-output-escaping="yes">&lt;</xsl:text> rows.length; rowIdx++)	{
						...
						for (colIdx = 0; colIdx <xsl:text disable-output-escaping="yes">&lt;</xsl:text> cols.length; colIdx++)	{
							...
							if (colIdx > 0) {
								...
								reportLink.attr("href", "/aim/viewNewAdvancedReport.do~view=reset~widget=false~ampReportId=2959");
						...
						percentColDomObject.attr("title", colTooltips[5]);
