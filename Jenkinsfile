        ...
        if (imageIds.equals("")) {
            withEnv(["PATH+MAVEN=${tool 'M339'}/bin"]) {
                try {
                    ...
                    // Build AMP
                    sh "cd amp && mvn -T 4 clean compile test war:exploded -Djdbc.user=amp -Djdbc.password=amp122006 -Djdbc.db=amp -Djdbc.host=db -Djdbc.port=5432 -DdbName=postgresql -Djdbc.driverClassName=org.postgresql.Driver -DbuildSource=${tag} -e"
