...
public class QuarterlyComparisonsWorker {
	...
	public static Collection getQuarterlyComparisons(FilterParams fp, BigDecimal totalCost) {
		if (logger.isDebugEnabled())
			logger.debug("getQuarterlyComparisons() passed filter parameters " + " ampFundingId=" + fp.getAmpFundingId());
		...
		while (iter.hasNext()) {
			...
			fillAdd(qc, qi, Constants.COMMITMENT,totalCost);
		}
		
		
		iter = disbursementOrders.iterator();
		
		merge(Constants.DISBURSEMENT_ORDER,totalCost);
		iter = disbursements.iterator();
		
		merge(Constants.DISBURSEMENT,totalCost);
		iter = expenditure.iterator();
		
		merge(Constants.EXPENDITURE,totalCost);
		
		Collections.sort(arrList, new QuarterlyInfoComparator());

		if (logger.isDebugEnabled())
			logger.debug("getQuarterlyComparisons() returning a collection of size " + (arrList != null ? arrList.size() : 0));
		return arrList;
	}

	public static void merge(int transactionType,BigDecimal totalCost) {
		if (logger.isDebugEnabled())
		...
		while (iter.hasNext()) {
			...
			if (qi.getAggregate() == 0) {
				...
				for (int i = 0; i < arrList.size(); i++) {
					...
					if (fy == qc.getFiscalYear() && fq == qc.getFiscalQuarter()) {
						...
						if (transactionType == Constants.DISBURSEMENT) {
							if (qc.getPlannedDisbursement() == 0.0 && qc.getActualDisbursement() == 0.0) {
						...
						if (transactionType == Constants.EXPENDITURE) {
							if (qc.getPlannedExpenditure() == 0.0 && qc.getActualExpenditure() == 0.0) {
				...
				if (!b) {
					QuarterlyComparison qc = new QuarterlyComparison();
					fillAdd(qc, qi, transactionType,totalCost);
				}
			}
	...
	public static void fillAdd(QuarterlyComparison qc, QuarterlyInfo qi, int transactionType,BigDecimal totalCost) {
		if (qi.getAggregate() == 0) {
			...
			if (transactionType == Constants.COMMITMENT) {
				...
				qc.setUncommittedBalance(totalCost.subtract(new BigDecimal(qi.getActualAmount())).doubleValue());
			}
			if (transactionType == Constants.DISBURSEMENT) {
