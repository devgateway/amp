...
CREATE TEMPORARY TABLE columns_ampclean AS (SELECT column_name,table_name FROM information_schema.columns WHERE table_schema='AMP_CLEAN');
CREATE TEMPORARY TABLE tables_ampclean AS (SELECT table_name from information_schema.tables WHERE table_schema='AMP_CLEAN');
