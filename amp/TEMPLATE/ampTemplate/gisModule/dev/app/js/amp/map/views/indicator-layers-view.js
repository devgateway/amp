...
module.exports = Backbone.View.extend({
	...
	showLayer: function(layer) {
		// only show if it is a model, noticed that this function is called
		// twice, with layer as model and layer as a collection
		if(layer.constructor.prototype instanceof Backbone.Model){
			...
			if (loadedLayer === 'loading') {
				...
				// prevent race condition 'show' calls adding multiple versions
				// of the layer.
			...
			layer.loadAll().done(function() {
				...
				if (layer.get('selected')) {
					...
					if (loadedLayer.bringToBack) {
						...
						// TODO: drs, very dirty way of hiding boundaries so
						// they don't hijack click events
						// I need to pull out boundaries into own view.
						self.admClustersLayersView.moveBoundaryBack();
					}
					self.trigger('addedToMap'); // TODO: better way. needed to
					// let map bring structures to
					// front.
				}
	...
	// used to hilight the geojson layer on click, show popup, and unhilight
	// after.
