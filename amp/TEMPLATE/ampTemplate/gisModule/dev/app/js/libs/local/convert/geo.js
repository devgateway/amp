/*
 * A helper class for converting ESRI Rest Geometry to GeoJSON Geometry and the
 * other way around.
 * source: https://github.com/Esri/geojson-utils/blob/master/src/jsonConverters.js
 */

define(function () {

  /*
   * determine if polygon ring coordinates are clockwise. clockwise signifies outer
   * ring, counter-clockwise an inner ring or hole. this logic was found at
  ...
  function esriGeometryToGcGeometry(esriGeom) {
    ...
    if (esriGeom) {
      ...
      if (esriGeom.x) {
        ...
        /*
         * array to hold the individual polygons. A polygon is an outer ring
         * with one or more inner rings the conversion logic assumes that the
         * Esri json is in the format of an outer ring (clockwise) followed by
         * inner rings (counter-clockwise) with a clockwise ring signalling the
  ...
  /*
   * Converts ESRI Rest Featureset, Feature, or Geometry to GeoJSON
   * FeatureCollection, Feature, or Geometry
   */
   function ESRIToGeoJSON(esriObject) {
   ...
   ************************************************/
  /*
   * compares a GeoJSON geometry type and ESRI geometry type to see if they
   * can be safely put together in a single ESRI feature. ESRI features must
   * only have one geometry type, point, line, polygon
   */
   ...
   * what the ESRI geometry should hold. Includes the ESRI geometry type and the
   * name of the member that holds coordinate information
   */
  ...
  /*
   * Convert GeoJSON polygon coordinates to ESRI polygon coordinates. GeoJSON
   * rings are listed starting with a singular outer ring. ESRI rings can be
   * listed in any order, but unlike GeoJSON, the ordering of vertices
   * determines whether it's an outer or inner ring. Clockwise vertices indicate
   * outer ring and counter-clockwise vertices indicate inner ring
   */
  ...
  /*
   * Converts GeoJSON geometry to ESRI geometry. The ESRI geometry is only
   * allowed to contain one type of geometry, so if the GeoJSON geometry is a
   * GeometryCollection, then only geometries compatible with the first geometry
   * type in the collection are added to the ESRI geometry
   *
  ...
  /*
   * Converts GeoJSON FeatureCollection, Feature, or Geometry to ESRI Rest
   * Featureset, Feature, or Geometry
   */
  function geoJSONToESRI(geoJsonObject) {
    var outObj,
      i,
      gcFeats,
      esriFeat;
  ...
  return {
    ESRIToGeoJSON: ESRIToGeoJSON,
    geoJSONToESRI: geoJSONToESRI
  };
