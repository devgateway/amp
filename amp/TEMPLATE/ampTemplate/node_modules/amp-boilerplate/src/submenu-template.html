<!-- if  URL blank, no href --!>
<% if (!obj.url) { obj.url ='#'; obj.className='dropdown-toggle', obj.dataToggle = 'data-toggle="dropdown"' } else {obj.className = ''; obj.dataToggle = ''} %>
<a href="<%= url %>" class="<%= className %>" <%= dataToggle %> >
...
<% if (obj.children && obj.children.length > 0) { %>
  ...
  <% _.each(obj.children, function(model) { %>
    ...
    <% if (model.children && model.children.length > 0) { model.dropdownSubmenu = 'dropdown-submenu' } %>
    ...
    <% if (model.children && model.children.length > 0) { %>
      ...
      <% _.each(model.children, function(submodel) { %>
        <li class="menu-item"><a href="#"><%= submodel.name %></a></li>
      <% }) %>
      </ul>
    <% } %>
