...
var FILTERS_WITH_TWO_LEVELS_ONLY = ['Beneficiary Agency', 'Contracting Agency', 'Responsible Organization']; //organization filters with two levels only
  ...
  url: Constants.ALL_FILTERS_URL,
    ...
    _buildTreeImplementation: function(self, attrs) {
	  ...
	  _createTree: function(data, definition) {
		  ...
		  var excludeFirstLevel = FILTERS_WITH_TWO_LEVELS_ONLY.indexOf(definition.name) > -1;		  			  
		  _.each(dataCopy, function(level1) {
				  ...
				  if (level1.children && level1.children.length > 0) {
					  ...
					  _.each(level1.children, function(level2) {
						  if (level2.children && level2.children.length > 0) {
							  level2.children = self._updateLevelData(level2, definition, Constants.LEVEL_THREE, filterIds[2]);	
							  
							  _.each(level2.children, function(level3) {
								  if (level3.children && level3.children.length > 0) {								  
									   level3.children = self._updateLevelData(level3, definition, Constants.LEVEL_FOUR, filterIds[3]);
									   _.each(level3.children, function(level4) {
										   if (level4.children && level4.children.length > 0) {
											   level4.children = self._updateLevelData(level4, definition, Constants.LEVEL_FIVE, filterIds[4]);
			...
			if (excludeFirstLevel == false || level1.id === UNDEFINED_ID) {
				if (level1.id === UNDEFINED_ID &&  (level1.filterId == null || level1.filterId == '')) {
					level1.filterId = filterIds[1];
				}
...
_hasValidChildren: function(node, definition, filterId) {
	...
	if (filterId  && filterId.includes(Constants.GROUP) && !filterId.includes(Constants.PLEDGES)) {
		if (node.children) {
