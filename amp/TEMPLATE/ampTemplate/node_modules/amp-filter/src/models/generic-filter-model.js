...
module.exports = BaseFilterModel.extend({
  ...
  initialize:function(options) {	
    ...
    if (options.tab === Constants.LOCATION && this.get('include-location-children') === undefined) {
  ...
  deserialize: function(listOfSelected) {
    ...
    reset: function () {
        ...
        if (tree && tree.get('numSelected') > 0) {
            ...
            if (tree.get('name') === Constants.LOCATIONS) {
                this._resetLocation();
            } else {
    ...
    _resetLocation: function () {
        // First check if the checkbox has been created or it will execute the wrong process and freeze the browser.
        if ($('.include-location-children').length === 0) {
            this.get('tree').set('selected', true);
            this.get('tree').set('selected', false);
            return;
        }
        // Current value, not previous.
        if ($('.include-location-children').is(':checked') === false) {
            // Process as if it where checked and then revert the change.
            $('.include-location-children').prop('checked', true);
            while (this.get('tree')._hasAnySelectedChildren()) {
                this.get('tree').set('selected', true);
                this.get('tree').set('selected', false);
            }
            $('.include-location-children').prop('checked', false);
        } else {
            while (this.get('tree')._hasAnySelectedChildren()) {
                this.get('tree').set('selected', true);
                this.get('tree').set('selected', false);
