...
TreeNodeModel = Backbone.Model.extend({
  ...
  serialize: function (options) {
    ...
    if (Constants.LOCATION === options.tab) {
      stopOnParent = false;
    }
    var tmpSerialized = {};
    var children = this.get('children');
    // AMP-28683: Before checking if it has children, check if the "parent" is selected.
    // IMPORTANT: we cant trust on "selected" field on branch nodes :(
    if (this.get('numSelected') === this.get('numPossible')) {
      tmpSerialized[this.get('filterId')] = (options.wholeModel ? [this] : [this.id]);
      if (!stopOnParent && children.length > 0) {
      ...
      if (children.length > 0) {
        if (Constants.LOCATION === options.tab) {
          ...
          if (this._isLocationAndDontIncludeChildren()) {
            this._serializeChildren(tmpSerialized, children, options);
  ...
  _isLocationAndDontIncludeChildren: function () {
    var result = false;
    if (this.get('name') === Constants.LOCATIONS || this.get('filterId') === Constants.COMMUNAL ||
        this.get('filterId') === Constants.DISTRICT ||
        this.get('filterId') === Constants.ZONE || this.get('filterId') === Constants.REGION ||
        this.get('filterId') === Constants.COUNTRY) {
