...
TreeNodeModel = Backbone.Model.extend({
  ...
  serialize: function (options) {
    ...
    if (isLocationAndDontIncludeChildren) {
      stopOnParent = false;
    }
    var tmpSerialized = {};
    ...
    if (options.includeUnselected ||
        (!isLocationAndDontIncludeChildren && this.get('numSelected') === this.get('numPossible')) ||
        (isLocationAndDontIncludeChildren && this.get('selected'))) {
  ...
  _onSelectChange: function (model, argument, options) {
    ...
    if (this.get('selected')) {
      ...
      if (this._isLocationAndDontIncludeChildren() && this._hasAnySelectedChildren()) {
        this.set('numSelected', 1);
      } else {
        this.set('numSelected', 0);
      }
    }
    if (!children.isEmpty() && !this._isLocationAndDontIncludeChildren()) {
  ...
  _isLocationAndDontIncludeChildren: function () {
    var result = false;
    if (this.get('name') === Constants.LOCATIONS || this.get('name') === Constants.PLEDGES_LOCATIONS ||
        this.get('filterId') === Constants.COMMUNAL ||
        this.get('filterId') === Constants.DISTRICT || this.get('filterId') === Constants.PLEDGES_DISTRICT ||
        this.get('filterId') === Constants.ZONE || this.get('filterId') === Constants.PLEDGES_ZONE ||
        this.get('filterId') === Constants.REGION || this.get('filterId') === Constants.PLEDGES_REGION ||
        this.get('filterId') === Constants.COUNTRY || this.get('filterId') === Constants.PLEDGES_COUNTRY) {
  ...
  _updateCount:function() {
    ...
    if (!children.isEmpty()) {
      ...
      if (this._isLocationAndDontIncludeChildren() && this.get('selected') && countTotal.selected === 0) {
