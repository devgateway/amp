...
var TreeNodeView = Backbone.View.extend({
  ...
  render:function(model) {
    this.model = model;
    this.model.set('hideCount', this.model._isLocationAndDontIncludeChildren());
    if (model) {
    ...
    if (!this.model.get('children').isEmpty()) {
      ...
      if (this.model.get('numSelected') > 0) {
        if (this.model.get('numSelected') < this.model.get('numPossible')) {
          if (this.model._isLocationAndDontIncludeChildren() && this.model.get('selected')) {
          ...
          this.$('> .node > .selectable').removeClass('half-fill');
          this.$('> .node > .selectable').addClass('selected');
          /* AMP-28966: when selecting a parent then select all children automatically */
      ...
      if (this.model.get('selected')) {
        ...
        this.$('> .node > .selectable').removeClass('selected');
        /* AMP-28966: When de-selecting a child then deselect the parent too, BUT dont trigger a full branch update
         or it will deselect all children. */
        this.model.get('parent').attributes.selected = false;
      }
    }
  },
