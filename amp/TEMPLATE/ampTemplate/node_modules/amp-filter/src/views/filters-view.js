 ...
 deserialize: function(filtersObject, options) {
    ...
    if (blob) {
      ...
      that.allFilters.each(function(filter) {
        ...
        if (filter.get('tab')  === Constants.LOCATION) {
            if (filtersObject.filters.includeLocationChildren !== undefined &&
                filtersObject.filters.includeLocationChildren !== null) {
                filter.get('tree').set('include-location-children', filtersObject.filters.includeLocationChildren);
            } else {
                filter.get('tree').set('include-location-children', true);
            }
        }      
      });
    } else {
  ...
  cancel: function() {
    ...
    getIncludeLocationChildren: function () {
        ...
        var locationFilter = this.allFilters.filter(function (filter) {
            return filter.get('tab') === Constants.LOCATION;
        })[0];
        if (locationFilter) {
            includeChildren = locationFilter.get('tree')._isLocationAndDontIncludeChildren() === false;
        }
