...
module.exports = Backbone.View.extend({
    ...
    getAppliedFiltersTree: function (serializedFilters, filter) {
        ...
        if (self.isNodeSelected(filter)) {
            ...
            if (children) {
                var root = self.createItemObject(filter.get('id'), filter.get('code'), filter.get('name'), [])
                children.each(function (child) {
                    // var child = tree;
                    // var filter = self.createItemObject(child.get('id'), child.get('code'), child.get('name'), []);
                    // var filter = self.createItemObject(tree.get('id'), tree.get('code'), tree.get('name'), []);
                    self.exploreTree(child, root, 1);
                });
        ...
        if (rootFilter.children.length > 0) {
            serializedFilters.filters.push(rootFilter.children);
        }
    },

    exploreTree: function(child, _filter, level) {
        var self = this;
        if (self.isNodeSelected(child)) {
            var node = self.createItemObject(child.get('id'), child.get('code'), child.get('name'), []);
            _filter.children.push(node);
        } else if (child.get('numSelected') > 0) {
            var node = self.createItemObject(child.get('id'), child.get('code'), child.get('name'), []);
            if (_filter.id !== node.id) {
                _filter.children.push(node); // todo: ver si en el primer nivel esto no hace falta hacerlo.
            }
            level++;
            child.get('children')
              .each(function (child_) {
                  self.exploreTree(child_, node, level);
              });
        }
    },
