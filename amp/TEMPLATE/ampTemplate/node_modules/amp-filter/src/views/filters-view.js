...
module.exports = Backbone.View.extend({
    ...
    formatDate: function (date) {
        let dateFormat = this.getDateFormat();  // get date format
        let parsedDate = $.datepicker.parseDate(this.PARAMS_DATE_FORMAT, date);  // parse date

        if (parsedDate) { // Ensure that date was parsed successfully
            return $.datepicker.formatDate(dateFormat, parsedDate).replace(/\//g, '-');
        } else {
    ...
    setDefaultDates: function (blob) {
        ...
        if (self.caller === Constants.CONTEXT_DASHBOARD) {
            return DateUtils.extractDates(self.settings, blob, 'dashboard-default-min-date', 'dashboard-default-max-date', this.formatDate);
        } else if (self.caller === Constants.CONTEXT_GIS) {
            return DateUtils.extractDates(self.settings, blob, 'gis-default-min-date', 'gis-default-max-date', this.formatDate);
        } else if (self.caller === Constants.CONTEXT_REPORT) {
            return DateUtils.extractDates(self.settings, blob, 'report-default-min-date', 'report-default-max-date', this.formatDate);
        }
        return blob;
    },
