...
module.exports = Backbone.View.extend({
    ...
    createItemObject(id, code, name, children) {
        return {id: id, code: code, name: name, children: children};
    },
    ...
    newSerialize() {
        ...
        this.allFilters.each(function (filter) {
            ...
            switch (filter.get('modelType')) {
                case 'TREE':
                    self.newSerializeTree(serializedFilters, filter);
                    break;
                case 'DATE-RANGE-VALUES':
                    self.newSerializeDateRange(serializedFilters, filter);
                    break;
                case 'YEAR-SINGLE-VALUE':
                    self.newSerializeDateYear(serializedFilters, filter);
                    break;
            }
        });
        console.error(serializedFilters);
    },

    newSerializeDateRange: function (serializedFilters, filter) {
        var self = this;
        var add = false;
        var rootFilter = {
            id: filter.get('id'),
    ...
    newSerializeDateYear: function (serializedFilters, filter) {
        var self = this;
    ...
    newSerializeTree: function (serializedFilters, filter) {
        var self = this;
        ...
        if (self.isNodeSelected(filter)) {
            ...
            var add = false;
            if (children) {
                children.each(function (child1) {
                    var _filter = null;
                    if (self.isNodeSelected(child1)) {
                        _filter = self.createItemObject(child1.get('id'), child1.get('code'), child1.get('name'), []);
                    } else if (child1.get('numSelected') > 0) {
                        _filter = self.createItemObject(child1.get('id'), child1.get('code'), child1.get('name'), []);
                        child1.get('children')
                          .each(function (child2) {
                              if (self.isNodeSelected(child2)) {
                                  _filter.children.push(self.createItemObject(child2.get('id'), child2.get('code'), child2.get('name'), []));
                              } else if (child2.get('numSelected') > 0) {
                                  var item2 = self.createItemObject(child2.get('id'), child2.get('code'), child2.get('name'), []);
                                  _filter.children.push(item2);
                                  child2.get('children')
                                    .each(function (child3) {
                                        if (self.isNodeSelected(child3)) {
                                            item2.children.push(self.createItemObject(child3.get('id'), child3.get('code'), child3.get('name'), []));
                                        } else if (child3.get('numSelected') > 0) {
                                            var item3 = self.createItemObject(child3.get('id'), child3.get('code'), child3.get('name'), []);
                                            item2.children.push(item3);
                                            child3.get('children')
                                              .each(function (child4) {
                                                  if (self.isNodeSelected(child4)) {
                                                      item3.children.push(self.createItemObject(child4.get('id'), child4.get('code'), child4.get('name'), []));
                                                  } else if (child4.get('numSelected') > 0) {
                                                      var item4 = self.createItemObject(child4.get('id'), child4.get('code'), child4.get('name'), []);
                                                      item3.children.push(item4);
                                                      child4.get('children')
                                                        .each(function (child5) {
                                                            if (self.isNodeSelected(child5)) {
                                                                item4.children.push(self.createItemObject(child5.get('id'), child5.get('code'), child5.get('name'), []));
                                                            } else if (child5.get('numSelected') > 0) {
                                                                var item5 = self.createItemObject(child5.get('id'), child5.get('code'), child5.get('name'), []);
                                                                item4.children.push(item5);
                                                                child5.get('children')
                                                                  .each(function (child6) {
                                                                      // to be implemented.
                                                                  });
                                                            }
                                                        });
                    ...
                    if (_filter) {
                        add = true;
                        rootFilter.children.push(_filter);
                    }
                });
            }
        }
