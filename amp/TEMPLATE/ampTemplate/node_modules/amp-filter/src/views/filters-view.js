    ...
    this.allFilters.each(function(filter) {
      ...
      if (filter.get('id') || filter.url) {   
        ...
        if (options.group === Constants.LOCATIONS) {
        	_.extend(serializedFilters.filters, {'include-location-children': filter.get('include-location-children') });
        }
      }
    });
 ...
 deserialize: function(filtersObject, options) {
    ...
    if (blob) {
      ...
      that.allFilters.each(function(filter) {
        ...
        if (filter.get('group')  === Constants.LOCATIONS && !_.isUndefined(blob['include-location-children']) ) {
        	filter.set('include-location-children', blob['include-location-children']);
        }
      });
    } else {
