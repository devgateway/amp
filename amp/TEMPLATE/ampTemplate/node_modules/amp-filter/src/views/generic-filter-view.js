...
module.exports = BaseFilterView.extend({
  ...
  onIncludeLocationChildrenChange: function(event) {
    if(event.target.checked) {
      ...
      this.model.set('include-location-children', true);
    } else {
      this.model._resetLocation();
      this.$('.count').addClass('hidden');
      this.model.set('include-location-children', false);
    }
  },
  ...
  _updateCountInMenu: function () {
    if (this.$titleEl && this.model.get('tree')) {
      if (this.model.get('tree').get('numSelected') === this.model.get('tree').get('numPossible') ||
        ...
        if (this.model.get('name') === Constants.LOCATIONS || this.model.get('name') === Constants.PLEDGES_LOCATIONS) {
  ...
  renderFilters:function() {
    ...
    this.model.getTree().then(function(treeModel) {     
      ...
      json.isLocationFilter = json.tab === Constants.LOCATION;
      self.$el.html(self.template(json));

      // add event listeners
      self.$('input.search-text').on('keyup', function(event) {self.searchKeyUp(event);});
      self.$('.include-location-children').on('click', function(event) {self.onIncludeLocationChildrenChange(event);}); //return false to stop page refresh.
