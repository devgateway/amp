...
module.exports  = Backbone.Collection.extend({
	...
	fromState: function(state) {
		...
		if (state !== undefined) {
			var savedCalendarId = _.find(state, function(item, key) {return key === Constants.SETTINGS.CALENDAR_ID});  
			var savedCalendar = _.find(_.find(this.models, function(item) {return item.id === Constants.SETTINGS.CALENDAR_ID}).get("options"), function(item2) {return item2.id === savedCalendarId});
			if (savedCalendar === undefined) {
				// This calendar was deleted, select another.
				state[Constants.SETTINGS.CALENDAR_ID] = _.find(this.models, function(item) {return item.id === Constants.SETTINGS.CALENDAR_ID}).get("options")[0].id;
			}
		}
