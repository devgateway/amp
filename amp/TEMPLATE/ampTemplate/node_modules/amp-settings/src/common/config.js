...
module.exports = {
		DASHBOARDS:{
		   'definition-url': '/rest/amp/settings',
		   parse : function(settings){
			   console.log('DASHBOARDS');
		    	return _(settings).chain()
				// mark weird options with non-int keys hidden
				.map(Utils.tagIf(function(setting) { return !Utils.isIntStr(setting.id); }, 'ignore'))
				// mark funding type setting hidden, since it's set per-chart...
				.map(Utils.tagIf(function(setting) { return setting.id === Constants.SETTINGS.FUNDING_TYPE_ID; }, 'ignore'))
				// ...but also flag it so the charts can see the the funding type options
				.map(Utils.tagIf(function(setting) { return setting.id === Constants.SETTINGS.FUNDING_TYPE_ID; }, 'ftype'))
