...
module.exports = Backbone.View.extend({
	...
	optionChanged : function(evt) {
		...
		if (settingID === Constants.YEAR_FROM_ID || settingID === Constants.YEAR_TO_ID) {
			if (settingID === 'from') {
				if (Number(selectedID) >= Number(self.appliedSettings['year-range'].to)
					...
					if (self.settingsSelections.from) {
						...
						$(evt.currentTarget).val(self.appliedSettings['year-range'].from);
					}
				} else {
					this.updateSelected(settingID, selectedID);
				}
			} else {
				if (Number(selectedID) <= Number(self.appliedSettings['year-range'].from)
					...
					if (self.settingsSelections.to) {
						...
						$(evt.currentTarget).val(self.appliedSettings['year-range'].to);
					}
				} else {
					this.updateSelected(settingID, selectedID);
				}
