...
module.exports = Backbone.View.extend({
	...
	updateUI : function() {
		...
		this.$('.settings').html('');
		var calendarSetting = this.settingsData.findWhere({
			id : Constants.SETTINGS.CALENDAR_ID
		});
		this.appendSelectSetting(calendarSetting);

		var currencySetting = this.settingsData.findWhere({
			id : Constants.SETTINGS.CURRENCY_ID
		});
		this.allCurrencies = currencySetting.get('options');
		this.appendSelectSetting(currencySetting);

		var fundingTypeSetting = this.settingsData.findWhere({
			id : Constants.SETTINGS.FUNDING_TYPE_ID
		});
		this.appendSelectSetting(fundingTypeSetting);

		var yearRangeSetting = this.settingsData.findWhere({
			id : Constants.SETTINGS.YEAR_RANGE_ID
		});
		if (yearRangeSetting) {
	...
	getCurrenciesByCalendar : function(calendarId) {
		var calendarCurrencies = this.settingsData.findWhere({
			id : Constants.SETTINGS.CALENDAR_CURRENCIES
		});
		var currencies = [];
	...
	optionChanged : function(evt) {
		...
		if (id === Constants.SETTINGS.CALENDAR_ID) {
			...
			this.$('#' + Constants.SETTINGS.CURRENCY_ID).empty();
			$.each(availableCurrenciesForCalendar, function(index, object) {
				this.$('#' + Constants.SETTINGS.CURRENCY_ID).append($("<option></option>").attr("value", object).text(_.find(self.allCurrencies, function(item) {
					return item.id === object
				}).name));
				self.settingsData.get(Constants.SETTINGS.CURRENCY_ID).get('options').push(_.find(self.allCurrencies, function(item) {
					return item.id === object
				}));
			});

			// select first currency on list
			this.$('#' + Constants.SETTINGS.CURRENCY_ID).val(availableCurrenciesForCalendar[0]);
	...
	applySettings : function() {
		...
		this.trigger('apply-settings', this.settingsData.toAPI());
	},
	close : function() {
		this.trigger('close');
	}
