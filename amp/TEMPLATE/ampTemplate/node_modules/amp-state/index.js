...
_.extend(State.prototype, Backbone.Events, {
  ...
  listenToURL: function(options) {
    console.log('listenToURL', options);
  ...
  urlMaybeLoad: function(hash) {
    ...
    if (hash.indexOf(this._urlPrefix) === 0) {
      var state_or_id = hash.split(this._urlPrefix)[1];
      if (state_or_id) {
        this[this.saved ? 'loadById' : 'load'](state_or_id);
      } else {
        this.reset();
      }
    } else if (hash === '') {
  ...
  toHash: function(state_or_id) {
    if (!_(this).has('_url')) {
      ...
      console.log("drs ",this._urlPrefix , state_or_id);
      return this._urlPrefix + state_or_id;
    }
  },

  loadById: function(id) {
    this.loadPromise = this.saved.load(id);
    this.loadPromise
  ...
  freeze: function(options) {
    ...
    if (options.toURL) {
      ...
      this._url.hash(this._urlPrefix + encodeURIComponent(statesBlob));
    }
