...
_.extend(State.prototype, Backbone.Events, {
  ...
  listenToURL: function(options) {
    ...
    if (this._url.hash()) { this.urlMaybeLoad(this._url.hash()); }  // initial state
  },
  ...
  urlMaybeLoad: function(hash) {
    ...
    if (hash.indexOf(this._urlPrefix) === 0) {
      ...
      if (state_or_id) {
        this.data ? this.loadById(state_or_id) : this.load(state_or_id);
      } else {
        this.reset();
      }
    } else if (hash === '') {
  ...
  freeze: function(options) {
    ...
    if (options.toURL) {
      ...
      this._url.hash(this._urlPrefix + statesBlob);
    }
