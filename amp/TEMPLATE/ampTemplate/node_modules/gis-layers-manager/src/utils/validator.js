...
validator.nameindicators = function(options){	
	...
	if(_.isUndefined(name[defaultLanguage]) || _.isEmpty(name[defaultLanguage])){		 
		 errorMessages.push(((options.languages.length > 1) ? (defaultLanguage + ' ' ) : '') + options.translator.translateSync('amp.gis-layers-manager:validation-NameRequired','Indicator Name is required'));
...
validator.definemeasurelabels = function(options){
	...
	if(_.isUndefined(unit[defaultLanguage]) || _.isEmpty(unit[defaultLanguage])){		 
		 errorMessages.push(((options.languages.length > 1) ? (defaultLanguage + ' ' ) : '') + options.translator.translateSync('amp.gis-layers-manager:validation-MeasureLabelRequired','Measure Label is required'));
		 deferred.resolve(errorMessages); 		
	}
...
validator.reimportfile = function(model,translator){
	var deferred = $.Deferred();
	var errorMessages = [];
	/*if(_.isUndefined(options.model.get('numberOfImportedRecords')) || options.model.get('numberOfImportedRecords') === 0){
