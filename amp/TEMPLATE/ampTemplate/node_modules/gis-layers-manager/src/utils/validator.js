...
validator.nameindicators = function(options){	
	...
	var errorMessages = [];
	var currentLanguage = options.settings.findWhere({id: 'language'}).get('defaultId');
	var name = _.isUndefined(options.model.get('name'))  ? {} : options.model.get('name');
	if(_.isUndefined(name[currentLanguage]) || _.isEmpty(name[currentLanguage])){
		 var translatedDefaultLang = options.translator.translateSync('amp.gis-layers-manager:message-currentLanguageLabel', 'The Language')
		 errorMessages.push(((options.languages.length > 1) ? (translatedDefaultLang + " '" + currentLanguage + "' " ) : '') + options.translator.translateSync('amp.gis-layers-manager:validation-NameRequired','Indicator Name is required'));
...
validator.definemeasurelabels = function(options){
	...
	if(_.isUndefined(unit[currentLanguage]) || _.isEmpty(unit[currentLanguage])){
	     var translatedDefaultLang = options.translator.translateSync('amp.gis-layers-manager:message-currentLanguageLabel','The Language')
		 errorMessages.push(((options.languages.length > 1) ? (translatedDefaultLang + " '" + currentLanguage + "' " ) : '') + options.translator.translateSync('amp.gis-layers-manager:validation-MeasureLabelRequired','Measure Label is required'));
		 deferred.resolve(errorMessages); 		
	}
