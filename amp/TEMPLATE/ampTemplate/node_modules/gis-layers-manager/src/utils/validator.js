...
validator.nameindicators = function(options){	
	...
	if(_.isUndefined(name[defaultLanguage]) || _.isEmpty(name[defaultLanguage])){
		 var translatedDefaultLang = options.translator.translateSync('amp.gis-layers-manager:message-DefaultLanguage', 'Default Language')
    ...
    NameUtil.isNameAvailable(options.model.get('name'), oldName).then(function(results){
    	...
    	_.each(keys,function(key){    		
    		if(results[key].error &&  results[key].error[ErrorCodes.NAME_EXISTS]){		   
...
validator.definemeasurelabels = function(options){
	...
	if(_.isUndefined(unit[defaultLanguage]) || _.isEmpty(unit[defaultLanguage])){
	     var defaultLanguageLabel = options.translator.translateSync('amp.gis-layers-manager:message-defaultLanguageLabel','The Default Language')
		 errorMessages.push(((options.languages.length > 1) ? (defaultLanguageLabel + " '" + defaultLanguage + "' " ) : '') + options.translator.translateSync('amp.gis-layers-manager:validation-MeasureLabelRequired','Measure Label is required'));
		 deferred.resolve(errorMessages); 		
	}
