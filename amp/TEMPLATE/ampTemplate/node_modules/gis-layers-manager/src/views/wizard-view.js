...
module.exports = Backbone.View.extend({
  ...
  next: function(e){     
	  ...
	  if(current === 'reimportfile'){
		  this.importFile(current);
	  }else{
  ...
  selectColors : function(e){	  
	  this.model.set('colorRampId', $(e.target).val());	
  },
  submitLayer: function(){
	  var self = this;
	  this.model.save(null, {
		  type: 'POST',
		  url:'/rest/postgis/indicator-layer',
  ...
  exportFile: function(){	  
	  ...
	  this.save().then(function(){
		  window.open('/rest/postgis/indicator-layer/export/' + self.model.get('admLevelId'), '_blank');	
	  });	    
  },
  save: function(){
	  var deffereds = [];
  ...
  importFile: function(currentStep){
	  ...
	  this.save().then(function(){
		  ...
		  $.ajax({
		         url : '/rest/postgis/indicator-layer/import',
		         type : 'POST',
		         data : formData,
		         processData: false, 
		         contentType: false,
		         success : function(data) {	        	 
		        	 self.$el.find('.nav-pills a[href="#' + Steps[currentStep].next + '"]').tab('show'); 
