...
module.exports = Backbone.View.extend({
  ...
  renderADMDropdown: function(){
	  ...
	  this.admLevels.each(function(admLevel){
		   ...
		   var translated = self.translator.translateSync('amp.gis-layers-manager:LayerLevel-' + admLevel.get('value').replace(' ',''), admLevel.get('label'));
	       self.$el.find(".adm-level").append($("<option></option>").attr("value",admLevel.get('id')).prop('selected', selected).text(translated));
	    });  
  },
  ...
  submitLayer: function(){
	  ...
	  Validator.final({model:this.model,translator:this.translator}).then(function(errors){
		  if(errors.length === 0) {
