define(['models/filter', 'collections/filters', 'translationManager', 'jquery'], function (Filter, Filters, TranslationManager, jQuery) {
    ...
    FilterUtils.extractSorters = function (content, columns, measures, hierarchies) {
        ...
        if (content !== null && content.models !== null) {
            ...
            _.each(columns.models, function (column) {
                if (_.find(hierarchies.models, function (item) {
                    return item.get('entityName').trim() === column.get('entityName');
                }) === undefined) {
        ...
        if (sorting.sidx !== undefined) {
            ...
            _.each(sidxColumns, function (item) {
                ...
                if (_.find(columns.models, function (item2) {
                        return item.trim() === item2.get('entityName')
                    }) === undefined
                    && _.find(measures.models, function (item2) {
                        return item.trim() === item2.get('entityName')
                    }) === undefined) {
    ...
    FilterUtils.extractFilters = function (filtersFromWidgetWithNames) {
        if (!app.TabsApp.filters) {
            app.TabsApp.filters = new Filters();
        }
        var filters = CommonFilterUtils.humanReadableFilters(filtersFromWidgetWithNames.filters);
        filters.forEach(function (item, i) {
            if (item.modelType === 'DATE-RANGE-VALUES' || item.modelType === 'YEAR-SINGLE-VALUE') {
                var dateFilter = FilterUtils._extractDateFilter(item.group, item);
                if (dateFilter) {
                    app.TabsApp.filters.add(dateFilter);
                }
            } else {
                var content = {
                    trnName: TranslationManager.getTranslated(item.group.replace(/-/g, " ")),
                    name: item.group,
                    values: item.items
                };
                app.TabsApp.filters.add(content);
            }
        });
