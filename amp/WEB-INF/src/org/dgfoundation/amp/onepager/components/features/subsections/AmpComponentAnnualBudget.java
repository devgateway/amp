...
public class AmpComponentAnnualBudget extends
		AmpSubsectionFeaturePanel<AmpActivityVersion> {

	protected AmpComponentFormTableAnnualBudget mtefTableFeature;

	public AmpComponentAnnualBudget(String id, 
			final IModel<AmpActivityVersion> model,String fmName) throws Exception {
		super(id, fmName, model);
		mtefTableFeature = new AmpComponentFormTableAnnualBudget("annualCostTable", "Annual Proposed Project Cost Table", model){
			@Override
        	protected void onFundingDetailChanged(AjaxRequestTarget target) {
        		send(findParent(AmpProposedProjectCost.class), Broadcast.BREADTH, new ProposedProjectCostUpdateEvent(target));
        	}
		};
		...
		AmpAjaxLinkField addMTEF=new AmpAjaxLinkField("addMTEF","Add Projection","Add Projection") {
			...
			protected void onClick(AjaxRequestTarget target) {
                ...
                if (mtefSet != null){
					...
					while (it.hasNext()) {
						AmpAnnualProjectBudget mtefItem = (AmpAnnualProjectBudget) it
								.next();
