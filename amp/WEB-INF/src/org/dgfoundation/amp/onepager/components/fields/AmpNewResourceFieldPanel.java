...
public class AmpNewResourceFieldPanel extends AmpFeaturePanel {
	...
	public AmpNewResourceFieldPanel(final String id, final IModel<AmpActivityVersion> model, final String fmName,
			final AmpResourcesFormTableFeature resourcesList, boolean newResourceIsWebLink) throws Exception {
		...
		final ResourceTranslationModel titleModel = new ResourceTranslationModel(new PropertyModel<String>(td, "title"),
				newResourceIdModel);
		final AmpTextFieldPanel<String> name = new AmpTextFieldPanel<String>("docTitle", titleModel, "Title",
				AmpFMTypes.MODULE, Boolean.TRUE);
		name.setTextContainerDefaultMaxSize();
		name.setOutputMarkupId(true);
		final ResourceTranslationModel descModel = new ResourceTranslationModel(
				new PropertyModel<String>(td, "description"), newResourceIdModel);
		final AmpTextAreaFieldPanel desc = new AmpTextAreaFieldPanel("docDesc", descModel, "Description", false, false,
				false);
		desc.setOutputMarkupId(true);
		final ResourceTranslationModel noteModel = new ResourceTranslationModel(new PropertyModel<String>(td, "note"),
				newResourceIdModel);
		...
		final AmpCategorySelectFieldPanel type = new AmpCategorySelectFieldPanel("docType",
				CategoryConstants.DOCUMENT_TYPE_KEY, new PropertyModel<AmpCategoryValue>(td, "type"), "Type", true,
				true);
		// FileUploadField file = new FileUploadField("file", new
		// AmpFileUploadModel(new PropertyModel<FileUpload>(td, "file")));
		...
		final AmpTextFieldPanel<String> webLink = new AmpTextFieldPanel<String>("webLink",
				new PropertyModel<String>(td, "webLink"), "Web Link", true, false);
		...
		final Form<?> form = new Form<Void>("form") {
			...
			protected void onSubmit() {
				...
				if (updateVisibility(td, resourceIsURL)) {
					...
					HashSet<TemporaryDocument> newItemsSet = getSession()
					...
					tmp.setTranslatedDescriptionList(
							getTranslationsForField(tmp.getNewTemporaryDocumentId(), "description"));
			...
			protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
				...
				if (updateVisibility(td, resourceIsURL)) {
					...
					target.appendJavaScript(
							"$('#uploadLabel').text('" + TranslatorWorker.translateText("No file chosen") + "');");

				}
				// target.add(form);
				// AmpNewResourceFieldPanel panel =
				// this.findParent(AmpNewResourceFieldPanel.class);
			...
			if (pathSelected) {
				...
				if (SettingsUtil.isLimitFileToUpload()) {
					try {
						...
						FileTypeValidationResponse validationResponse = mimeTypeManager.validateFileType(is,
	...
	private List<ResourceTranslation> getTranslationsForField(String id, String field) {
		...
		HashMap<String, ResourceTranslationStore> translationMap = Session.get()
				.getMetaData(OnePagerConst.RESOURCES_TRANSLATIONS);
