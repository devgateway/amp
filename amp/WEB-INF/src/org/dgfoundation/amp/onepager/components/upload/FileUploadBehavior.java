...
/**
 * Contributes all CSS/JS resources needed by
...
public class FileUploadBehavior extends Behavior {
	...
	/**
	 * The name of the request parameter used for the multipart Ajax request
	 */
	...
	/**
	 * Configures the connected component to render its markup id because it is
	 * needed to initialize the JavaScript widget.
	 * 
	 * @param component
	 */
	@Override
	...
	public void renderHead(final Component component, IHeaderResponse response) {
		...
		// new CssResourceReference(FileUploadBehavior.class,
		// "jquery.fileupload-ui.css")));
		// response.render(JavaScriptHeaderItem.forReference(
		// new JavaScriptResourceReference(FileUploadBehavior.class,
		// "jquery.fileupload-ui.js")));
		response.render(JavaScriptHeaderItem.forReference(
				new JavaScriptResourceReference(FileUploadBehavior.class, "jquery.ui.widget.js"),
				String.valueOf(System.currentTimeMillis()) + "a", true));
		response.render(JavaScriptHeaderItem.forReference(
				new JavaScriptResourceReference(FileUploadBehavior.class, "jquery.iframe-transport.js"),
				String.valueOf(System.currentTimeMillis()) + "b", true));
		response.render(JavaScriptHeaderItem.forReference(
				new JavaScriptResourceReference(FileUploadBehavior.class, "jquery.fileupload.js"),
				String.valueOf(System.currentTimeMillis()) + "c", true));

		String uploadUrl = RequestCycle.get().getUrlRenderer().renderFullUrl(Url
				.parse(component.urlFor(new FileUploadResourceReference(activityId, fileItemModel), null).toString()));
		String markupId = component.getMarkupId();

		Integer lMaxFileSizeGS = SettingsUtil.getMaximunFileSize();
		...
		variables.put("uploadFailedTooBigMsg",
				TranslatorUtil.getTranslatedText("The file size limit is {size} MB. This file exceeds the limit.")
						.replace("{size}", (lMaxFileSizeGS == null ? "" : lMaxFileSizeGS.toString())));
		variables.put("uploadMaxFileSize", Long.toString(Bytes.megabytes(lMaxFileSizeGS).bytes()));
		...
		response.render(OnLoadHeaderItem
				.forScript("setupFileUpload('#" + markupId + "', '" + uploadUrl + "', '" + PARAM_NAME + "');"));
	}
}