...
public class ReportsUtil {
     ...
     *                   where: <br>
     *                   <dl>
     *                     <dt>name</dt>          <dd>mandatory to be provided for custom reports, otherwise is
     *                     skipped</dd>
     ...
     *                     <dt>add_columns</dt>   <dd>optional, a list of columns names to be added to the report
     *                     configuration</dd>
     *                     <dt>columns_with_ids</dt>  <dd>optional, a list of columns names that should also provide
     *                     ids</dd>
     *                     <dt>add_hierarchies</dt>  <dd>optional, a list of hierarchies to be added to the report
     *                     configuration</dd>
     *                     <dt>add_measures</dt>  <dd>optional, a list of measures to be added to the report
     *                     configuration</dd>
     *                     <dt>raw_values</dt>       <dd>optional, a list that configures which elements must provide
     *                     raw (unformatted)
     *                                               values, with possible options (default []):
     *                                               "M": measures </dd>
     *                     <dt>show_empty_rows</dt> <dd>optional, default false, to show rows with empty measures
     *                     amounts</dd>
     *                     <dt>show_empty_cols</dt> <dd>optional, default false, to show full column groups (by
     *                     quarter, year)
     *                                                with empty measures amounts</dd>
     *                     <dt>forceHeaders</dt>  <dd>optional, flag, if the report query returns empty response
     *                                                the list of column headers is populated from the request.
     *                                                Default is false</dd>
     *                     <dt>settings</dt>      <dd>Report settings</dd>
     *                     <dt>reportType</dt>    <dd>can be on of "D" (Donor), "C" (Component), "P" (Pledge) report
     *                     type.
     *                                            Default is "D" if not provided.</dd>
     *                     <dt>projectType</dt>   <dd>an optional list of projects, mainly used to change usual
     *                     default project
     *                                            types included in the report. The list option per report type:
     *                                            <br>"D" : ["A", "S", "P"], where default is ["A", "S"] if both are
     *                                            reachable
     *                                            <br>"C" : ["A", "S"], where default is ["A", "S"]
     *                                            <br>"P" : ["P"], where default is ["P"]
     *                                            <br>where "A" - standard activity, "S" - SSC Activity, "P" -
     *                                            pledge</dd>
    ...
    @Nullable
    public static ReportArea getReportAreaBasedOnPagination(int recordsPerPage, int start,
    ...
    public static ReportSpecification update(ReportSpecification spec, ReportFormParameters formParams) {
        ...
        AmpFiscalCalendar oldCalendar = specImpl.getSettings() == null ? null
                : (AmpFiscalCalendar) specImpl.getSettings().getCalendar();
    ...
    public static void configureFilters(ReportSpecificationImpl spec, Map<String, Object> filterMap,
        ...
        if (filterMap != null) {
            ...
            if (spec.getFilters() != null) {
                ...
                // for now, if this is a calendar setting, let's check if any filters still exist and needs to be
                // converted
                if (spec.getSettings().getCalendar() != oldCalendar
                        && newFilters.getFilterRules().get(new ReportElement(ElementType.DATE)) != null) {
                    // for now we always use Gregorian, we'll update workflow when we'll add "calendar per date"
                    // support
    ...
    /**
     * copies entries which are not enclosed in the Filter widget but should be kept post-filter-application from
     * oldFilters to newFilters<br />
    ...
    protected static AmpReportFilters copyStickyMtefEntries(AmpReportFilters oldFilters, AmpReportFilters newFilters) {
        ...
        // copy MTEF-hacky entries from old widget to new widget, since these are supposed to be sticky (not present
        // in the filter form)
    ...
    public static void configureSSCWorkspaceFilter(ReportSpecificationImpl spec, boolean add) {
        ...
        if (filters == null) {
            AmpFiscalCalendar calendar = spec.getSettings() == null ? null
                    : (AmpFiscalCalendar) spec.getSettings().getCalendar();
            ...
            for (SortParam sort : sortingConfig) {
                ...
                if (errors.size() > 0) {
                    ...
                    int rootType = isTotals ? SortingInfo.ROOT_PATH_TOTALS : (isFunding
                            ? SortingInfo.ROOT_PATH_FUNDING : SortingInfo.ROOT_PATH_NONE);
    ...
    public static String exportToMap(final ReportConfig config, final Long reportId) {
        ...
        MEASURE_TEST:
        for (String measureName : GisConstants.FUNDING_TYPES) {
