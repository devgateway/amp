			...
			if(event.equals("addSector"))
			{
				...
				if(addSectorForm.getLevelType().equals("sector"))
				{
					...
					if(sectorStatus == 2) {
						...
						ActionErrors errors = new ActionErrors();
		        		errors.add("title", new ActionError("error.aim.addSubSector.wrongCode", TranslatorWorker.translateText("The code of the sub sector already exist for this scheme. Please enter another code",locale,siteId)));
		        		if (errors.size() > 0) {
		        			saveErrors(request, errors);
		        			return mapping.findForward("forwardSector");
		        		}
