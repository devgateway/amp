...
public class ViewQuarterlyByProject extends Action
{
...
//		formBean.setAdjustmentFlag("false");
		...
		filters=DbUtil.getTeamPageFilters(ampTeamId,Constants.QUARTERLYBYPROJECT);
...
//				ampAdjustmentId=Constants.ACTUAL;
		...
		dbReturnSet=ReportUtil.getAmpQuarterlyReportByProject(ampTeamId,fromYr,toYr,perspective,ampCurrencyCode,ampModalityId,ampStatusId,ampOrgId,ampSectorId,fiscalCalId,startDate,closeDate,region);
		int check = dbReturnSet.size();
		ArrayList checkList = dbReturnSet;
		logger.info( checkList.size() + "  : ArrayList :" + dbReturnSet.size());
		...
		logger.info(" YR = "+formBean.getFiscalYrRange());
...
//---------------------------------------------------		
		...
		formBean.setReportName("Quarterly Report By Project");
