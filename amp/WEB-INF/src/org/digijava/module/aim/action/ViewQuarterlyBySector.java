...
public class ViewQuarterlyBySector extends Action{
	
	private static Logger logger = Logger.getLogger(ViewbySectorbyProject.class);
...
//		formBean.setAdjustmentFlag("false");
		...
		filters=DbUtil.getTeamPageFilters(ampTeamId,Constants.QUARTERLYBYSECTOR);
...
/*			if(filters.indexOf(Constants.ACTUAL_PLANNED)!=-1)
			{
				...
				setFilters = setFilters + " ADJUSTMENT -";
				filterCnt++;
			}*/
...
//				ampAdjustmentId=Constants.ACTUAL;
		...
		dbReturnSet=ReportUtil.getAmpQuarterlyReportBySector(ampTeamId,fromYr,toYr,perspective,ampCurrencyCode,ampModalityId,ampStatusId,ampOrgId,ampSectorId,fiscalCalId,startDate,closeDate,region);
