...
public class CVRService {
	private static Logger logger = Logger.getLogger(CVRService.class);
	...
	public static Map<String, List<CompareOutput>> compareActivities(Long activityOneId) throws Exception {
		...
		// Get modified date and activity id for all versions
		...
		for (AmpActivityVersion activity : activities) { 
			dates.add(activity.getModifiedDate());
		...
		Long prevActivityId = Long.parseLong(activitiesIds.get(dates.indexOf(ampActivityOne.getModifiedDate()) - 1).toString());

		AmpActivityVersion ampActivityTwo = (AmpActivityVersion) session.load(AmpActivityVersion.class, prevActivityId);
		...
		ActivityHistory auditHistory1 = CVRService.getAuditHistory(ampActivityOne);
		ActivityHistory auditHistory2 = CVRService.getAuditHistory(ampActivityTwo);
				...
				if (!(auxResult1 == null && auxResult2 == null)) {
					if ((auxResult1 != null && auxResult2 == null) || (auxResult1 == null && auxResult2 != null)
						...
						if (auxReturnType.getName().equals("java.util.Date")
							...
							String aux1String = CVRService.getStringOrEmpty(auxResult1);
							String aux2String = CVRService.getStringOrEmpty(auxResult2);
		...
		return CVRService.groupOutputCollection(outputCollection);
	}
	...
	private static void addAsDifferentIfNnoPresent(List<CompareOutput> outputCollection, Field[] fields, int i,
		...
		while (iter1.hasNext()) {
			...
			if (coincidence == 0) {
				CompareOutput auxOutput = new CompareOutput(auxAnnotation.fieldTitle(), new String[] { auxValue1, "" },
						fields[i], new Object[] { auxObject1, null }, false, false);
				outputCollection.add(auxOutput);
			}
		}
