...
//					+ " where id > 9000 ")
	...
	private static void removeFromTemplate(AmpObjectVisibility fme){
		for (Iterator iterator = fme.getTemplates().iterator(); iterator.hasNext();) {
			AmpTemplatesVisibility atv = (AmpTemplatesVisibility) iterator.next();
			if (fme instanceof AmpModulesVisibility) {
				AmpModulesVisibility module = (AmpModulesVisibility) fme;
				atv.getItems().remove(module);

				for (Iterator mIter = module.getSubmodules().iterator(); mIter.hasNext();) {
					removeFromTemplate((AmpObjectVisibility)mIter.next());
				}
				for (Iterator fIter = module.getItems().iterator(); fIter.hasNext();) {
					removeFromTemplate((AmpObjectVisibility)fIter.next());
				}
			} else if (fme instanceof AmpFeaturesVisibility){
				AmpFeaturesVisibility feature = (AmpFeaturesVisibility)fme;
				atv.getFeatures().remove(feature);
				for (Iterator fiIter = feature.getItems().iterator(); fiIter.hasNext();) {
					removeFromTemplate((AmpObjectVisibility)fiIter.next());
				}
			} else if (fme instanceof AmpFieldsVisibility){
				AmpFieldsVisibility field = (AmpFieldsVisibility)fme;
				atv.getFields().remove(field);
			}
		}
//		fme.getTemplates().clear();
	}
