...
public class FinancingBreakdownWorker {
	...
	public static String getDonor(AmpFunding ampFunding) {
		...
		if (logger.isDebugEnabled())
			logger.debug("getDonor() of worker class returning organisation " + ampOrganisation.getName());
	...
	public static String getTotalDonorFund(FilterParams fp, boolean isDebug) {
		if (logger.isDebugEnabled())
			logger.debug("getTotalDonorFund() with FilterParams " + " transactionType=" + fp.getTransactionType() + " ampFunding=" + fp.getAmpFundingId());
	...
	public static BigDecimal getTotalActualCommitment(FilterParams fp, boolean isDebug) {
		DecimalWraper total = new DecimalWraper();
		Collection<YearlyInfo> c = YearlyInfoWorker.getYearlyInfo(fp);
		if (c.size() != 0) {
			total = Logic.getInstance().getTotalDonorFundingCalculator().getTotalDonorFunding(c, fp);
	...
	public static String getTotalDonorDisbursement(Long ampFundingId, double fromExchangeRate, double toExchangeRate) {
	...
	public static String getTotalDonorExpenditure(Long ampFundingId, double fromExchangeRate, double toExchangeRate) {
	...
	public static String getOverallTotal(Collection c, int type, boolean isDebug) {
		...
		while (iter.hasNext()) {
			...
			}else if (type == Constants.PROJECT_COST) {
					s1 = financingBreakdown.getTotalCost();
					total = FormatHelper.parseDouble(s1);
					break;
			}
		}
	...
	public static Collection<FinancingBreakdown> getFinancingBreakdownList(Long ampActivityId, Collection<AmpFunding> ampFundings, FilterParams fp, boolean isDebug) {
		...
		if (ampFundings != null) {
			...
			while (iter.hasNext()) {
				...
				financingBreakdown.setSignatureDate(DateConversion.ConvertDateToString(ampFunding.getSignatureDate()));
				...
				String unDisbursed = "";
				if (isDebug) {
					unDisbursed = "(" + totalDonorCommitment + ")" + "-" + "(" + totalDonorDisbursement + ")";
				} else {
				...
				String unExpended = "";
				if (isDebug) {
					unExpended = "(" + totalDonorDisbursement + ")" + "-" + "(" + totalDonorExpenditure + ")";
				} else {
				...
				cost=CurrencyWorker.convert(cost, fp.getCurrencyCode());
				financingBreakdown.setTotalCost(FormatHelper.formatNumber(cost));
				BigDecimal actualCommitment=FormatHelper.parseBigDecimal(totalDonorCommitment);	
				financingBreakdown.setUncommittedbalance(FormatHelper.formatNumber(cost.subtract(actualCommitment)));
			}
		} else {
		}
		if (logger.isDebugEnabled())
