...
public class AmpBackgroundActivitiesValidator
{
	public final static String AMP_VALIDATOR_USER_EMAIL = "amp_validator@amp.org";
	public final static String AMP_VALIDATOR_USER_PASSWORD = "DuMmY_PaSsWoRd_NoBoDy_ShOuLd_UsE_iT";
	public final static String AMP_VALIDATOR_ORGANIZATION_NAME = "Development Gateway Internal Org";
	public final static String AMP_VALIDATOR_ORGANIZATION_ACRONYM = "DGIO";
	public final static String AMP_VALIDATOR_ORGANIZATION_CODE = "DGIO";
	...
	{
		AmpOrganisation organization = DbUtil.getOrganisationByName(AMP_VALIDATOR_ORGANIZATION_NAME);
		if (organization != null)
			return organization;

		organization = new AmpOrganisation();
		organization.setName(AMP_VALIDATOR_ORGANIZATION_NAME);
		organization.setAcronym(AMP_VALIDATOR_ORGANIZATION_ACRONYM);
		...
		organization.setOrgCode(AMP_VALIDATOR_ORGANIZATION_CODE);
	...
	protected static void createAmpValidatorUser() throws DgException
	{
		User user = new User(AMP_VALIDATOR_USER_EMAIL.toLowerCase(), "Automatic", "Validation");
		...
		// set password
		user.setPassword(AMP_VALIDATOR_USER_PASSWORD);
		user.setSalt(AMP_VALIDATOR_USER_PASSWORD);
	...
	public static User createActivityValidatorUserIfNeeded() throws Exception
	{
		User user = UserUtils.getUserByEmail(AMP_VALIDATOR_USER_EMAIL);
		if (user != null)
			return user;

		createAmpValidatorUser();
		user = UserUtils.getUserByEmail(AMP_VALIDATOR_USER_EMAIL);
		if (user == null)
			throw new RuntimeException("bug creating AMP_VALIDATOR_user");
		return user;
	}
	
	/**
	 * gets or created an AmpTeamMember linked to a given workspace, which belongs to the {@link #AMP_VALIDATOR_USER_EMAIL}
	 * @param team the workspace
	 * @return
	 * @throws Exception
	 */
	public static AmpTeamMember createActivityValidatorTeamMemberIfNeeded(AmpTeam team) throws Exception
	{
		User user = createActivityValidatorUserIfNeeded();
