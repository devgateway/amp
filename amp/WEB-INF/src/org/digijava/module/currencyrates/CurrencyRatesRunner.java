...
public class CurrencyRatesRunner {
    private static Logger logger = LoggerFactory.getLogger(CurrencyRatesRunner.class);
    ...
    public CurrencyRatesRunner(int hour_init, int min_init, int hour_delay, int min_delay) {
    ...
    public void restart(int hour, int min, int hour_repeat, int min_repeat) {
        ...
        simpleTrigger = TriggerBuilder.newTrigger()
                ...
                .withSchedule(SimpleScheduleBuilder.simpleSchedule()
                        .withIntervalInMilliseconds(getDelay())
                        .repeatForever())
                .build();

        try {
            scheduler.rescheduleJob(TriggerKey.triggerKey(trgName, trgGroupName), simpleTrigger);
    ...
    private long getDelay() {
        if (hour_delay < 0 || hour_delay > 24 || min_delay < 0 || min_delay > 60 || (hour_delay <= 0 && min_delay <= 0)) {
    ...
    private Date getTomorrow(int hour, int min) {
        ...
        tomorrow.add(Calendar.DATE, fONE_DAY);
        Calendar result = new GregorianCalendar(tomorrow.get(Calendar.YEAR), tomorrow.get(Calendar.MONTH),
                tomorrow.get(Calendar.DATE), hour, min);
        return result.getTime();
    }
    ...
    private Date getToday(int hour, int min) {
        ...
        Calendar result = new GregorianCalendar(today.get(Calendar.YEAR), today.get(Calendar.MONTH),
                today.get(Calendar.DATE), hour, min);
        return result.getTime();
    }
    ...
    private Date initDate() {
        ...
        GregorianCalendar gc1 = new GregorianCalendar(Calendar.YEAR, Calendar.MONTH, Calendar.DAY_OF_MONTH,
                hour_update, min_update, 0);
        GregorianCalendar gc2 = new GregorianCalendar(Calendar.YEAR, Calendar.MONTH, Calendar.DAY_OF_MONTH,
                currentHour, currentMin, Calendar.SECOND);
