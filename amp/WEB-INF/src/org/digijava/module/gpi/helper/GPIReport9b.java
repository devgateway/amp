...
public class GPIReport9b extends GPIAbstractReport {

    private static Logger logger = Logger.getLogger(GPIReport9b.class);
    private final String reportCode = GPIConstants.GPI_REPORT_9b;
            ...
            try {
                ...
                while (iter.hasNext()) {
                    ...
                    if (useFundingDetail) {
                        ...
                        boolean[] answers = GPIUtils.getSurveyAnswers(GPIConstants.GPI_REPORT_9b, arrayResponses);
                        if (answers != null) {
                            float coefficient = 0;
                            for (int i = 0; i < 4; i++) {
                                if (answers[i]) {
                            ...
                            // activityform saves the survey automatically even
                            // with no responses.
    ...
    public Collection<GPIReportAbstractRow> reportPostProcess(Collection<GPIReportAbstractRow> baseReport, int startYear, int endYear) throws Exception {
        ...
        Comparator compareRows = new Comparator() {
            ...
            public int compare(Object o1, Object o2) {
                GPIReport9bRow aux1 = (GPIReport9bRow) o1;
                GPIReport9bRow aux2 = (GPIReport9bRow) o2;
        ...
        while (iter.hasNext()) {
            ...
            if (auxGroup.getAmpOrgGrpId().equals(row.getDonorGroup().getAmpOrgGrpId())) {
                ...
                if (row.getYear() == currentYear) {
                    ...
                    GPIReport9bRow newRow = new GPIReport9bRow();
                    if(auxColumn1 != null) {
        ...
        for (GPIReportAbstractRow row : newList) {
            GPIReport9bRow auxRow = (GPIReport9bRow) row;
    ...
    private Collection<GPIReportAbstractRow> calculatePercentages(Collection<GPIReportAbstractRow> coll, int startYear, int endYear) throws Exception {
        ...
        while (iterColl.hasNext()) {
            ...
            GPIReport9bRow auxRow = (GPIReport9bRow) iterColl.next();
            if(auxRow.getColumn2() != null) {
                if (auxRow.getColumn2().doubleValue() > 0) {
                    if(auxRow.getColumn1() == null) {
        ...
        for (int i = 0; i < endYear + 1 - startYear; i++) {
            GPIReport9bRow auxRow = new GPIReport9bRow();
    ...
    private Collection<GPIReportAbstractRow> addMissingYears(Collection<GPIReportAbstractRow> coll, int startYear, int endYear) throws Exception {
        ...
        while (iterRet.hasNext()) {
            GPIReport9bRow rowRet = (GPIReport9bRow) iterRet.next();
            Iterator iterOrigen = coll.iterator();
            while (iterOrigen.hasNext()) {
                GPIReport9bRow rowOrigen = (GPIReport9bRow) iterOrigen.next();
