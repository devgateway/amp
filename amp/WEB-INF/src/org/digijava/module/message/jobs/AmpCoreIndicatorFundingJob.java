...
public class AmpCoreIndicatorFundingJob extends ConnectionCleaningJob implements StatefulJob {
    ...
    public void executeInternal(JobExecutionContext context) throws JobExecutionException {
        ...
        if (defCountry == null) {
            logger.error("Default country ISO not found");
            return;
        }
        if (defCountry.equalsIgnoreCase("zz")) // TODO: 25/12/2024  add condition for ecowas
        {
            executeInternalForMulticountry(context);
        } else {
            executeInternalForSingleCountry(context);
        }
    }

    public void executeInternalForSingleCountry(JobExecutionContext context) throws JobExecutionException {
    ...
    public void executeInternalForMulticountry(JobExecutionContext context) throws JobExecutionException {
        //TODO move to MEService
        List<CoreIndicatorProgressDTO> resp = new ArrayList<>();
        StringBuilder query = new StringBuilder();
        query.append("        SELECT cv.id                                                                                AS core_type_id,  ");
        query.append("        cv.category_value                                                                    AS core_type_name,  ");
        query.append("        al.location_id                                                                       AS country_id,  ");
        query.append("        cvl.location_name                                                                    AS country_name,  ");
        query.append("        org.amp_org_id                                                                       AS donor_id,  ");
        query.append("        org.name                                                                             AS donor_name,  ");
        query.append("        i.indicator_id                                                                       AS indicator_id,  ");
        ...
        query.append("        JOIN amp_activity_location al ON (ic.activity_location = al.amp_activity_location_id and  ");
        query.append("        iv.activity_location = al.amp_activity_location_id)  ");
