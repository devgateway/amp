...
public class XmlPatcherNativeLangWorker extends XmlPatcherLangWorker {
	 ...
	 * @param log
	 */
	public XmlPatcherNativeLangWorker(Object entity, XmlPatchLog log) {
		super(entity, log);
	...
	protected boolean process() throws XmlPatcherWorkerException {
		try {
			...
			if (!XmlPatcherUtil.isSQLCompatible(getLang().getValue()))
				return false;
			...
			StringTokenizer stok = new StringTokenizer(getLang().getValue().trim(),
					getLang().getDelimiter());
	 ...
	 * 
	 * @see org.digijava.module.xmlpatcher.workers.XmlPatcherWorker#runTimeCheck()
	 */
	@Override
	protected boolean runTimeCheck() throws XmlPatcherWorkerException {
		...
		for (int i = 0; i < XmlPatcherConstants.ScriptLangs.natives.length; i++) {
			if (XmlPatcherConstants.ScriptLangs.natives[i].equals(getLang()
					.getType().value()))
				return langFound = true;
		}
		if (!langFound)
			throw new XmlPatcherLangWorkerException(
					"Unsupported native language " + getLang().getType());
