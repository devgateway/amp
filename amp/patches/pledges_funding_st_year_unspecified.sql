...
from ((((((((`amp_funding_pledges` `f`
     ...
     left join `amp_category_value` `cval` on ((`cval`.`id` =
      `fd`.`type_of_assistance`)))
     join `amp_currency` `c`)
     join `amp_organisation` `d`)
     join `amp_org_group` `b`)
     join `amp_org_type` `ot`)
     left join `amp_category_value` `cval2` on ((`cval2`.`id` =
      `fd`.`aid_modality`)))
     join `amp_funding_detail` `afd` on ((`afd`.`pledge_id` = `f`.`id`)))
where ((`c`.`amp_currency_id` = `fd`.`currency`) and
...
select `f`.`id` AS `pledge_id`,
       ...
       str_to_date(concat('01/', '02/', `fd`.`year`), '%d/%m/%Y') AS
        `transaction_date`,
