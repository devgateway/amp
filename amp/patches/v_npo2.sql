...
on((`b6`.`amp_theme_id` = `b5`.`parent_theme_id`))) left join `amp_theme` `b7`
 on((`b7`.`amp_theme_id` = `b6`.`parent_theme_id`))) where (`getProgramSettingId`(`a`.`amp_program_id`) = 1);
...
when `v_nationalobjectives_all_level`.`l8` then `v_nationalobjectives_all_level`.`n8` end) AS `name`,
(case 0 
when `v_nationalobjectives_all_level`.`l1` then `v_nationalobjectives_all_level`.`amp_program_id1` 
when `v_nationalobjectives_all_level`.`l2` then `v_nationalobjectives_all_level`.`amp_program_id2` 
when `v_nationalobjectives_all_level`.`l3` then `v_nationalobjectives_all_level`.`amp_program_id3` 
when `v_nationalobjectives_all_level`.`l4` then `v_nationalobjectives_all_level`.`amp_program_id4` 
when `v_nationalobjectives_all_level`.`l5` then `v_nationalobjectives_all_level`.`amp_program_id5` 
when `v_nationalobjectives_all_level`.`l6` then `v_nationalobjectives_all_level`.`amp_program_id6` 
when `v_nationalobjectives_all_level`.`l7` then `v_nationalobjectives_all_level`.`amp_program_id7` 
when `v_nationalobjectives_all_level`.`l8` then `v_nationalobjectives_all_level`.`amp_program_id8` end) AS `amp_program_id`,
sum(`v_nationalobjectives_all_level`.`program_percentage`) AS `program_percentage` 
from `v_nationalobjectives_all_level`  
group by amp_activity_id, amp_program_id having (`name` is not null);
...
update amp_columns set relatedContentPersisterClass="org.digijava.module.aim.dbentity.AmpTheme" where columnName like "National Planning Objectives%";
