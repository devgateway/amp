...
<div id='pledge_form_funding_data'>
	<div class="highlight text-center h5 bold"><digi:trn key="aim:pledgeInformation">Pledge Information</digi:trn></div>
					...
					<c:forEach var="fund" items="${pledgeForm.selectedFunding}" varStatus="index">
						...
						<tr>
							<td class="text-center">
								<c:set var="select_id" value="pledgeTypeDropDown_${indexLoc}" /><c:set var="extra_tags"></c:set>
							...
							<c:if test="${pledgeForm.fundingShowTypeOfAssistance}"><td class="text-center">
								<c:set var="select_id" value="pledgeTypeOfAssistanceDropDown_${indexLoc}" /><c:set var="extra_tags"></c:set>
							...
							<c:if test="${pledgeForm.fundingShowAidModality}"><td class="text-center">
								<c:set var="select_id" value="pledgeAidModalityDropDown_${indexLoc}" /><c:set var="extra_tags"></c:set>
								<c:set var="select_values" value="${pledgeForm.aidModalities}" />
								<c:set var="select_init_value" value="${fund.aidModalityId}" />
								<%@include file="renderShimList.jspf" %>
							</td></c:if>														
							<td><button type="button" onclick="programsController.onDelete(this);" class="btn btn-danger btn-xs"><digi:trn>Delete</digi:trn></button></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
