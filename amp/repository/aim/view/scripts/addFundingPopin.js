...
function validateProjection (errorMsg) {
	...
	var numProjections	= document.aimEditActivityFormPopin.numProjections.value;
	...
	for (i=0; i<numProjections; i++) {
		...
		var elements	= document.aimEditActivityFormPopin.elements;
...
function validateFundingTrn(errmsg1,errmsg2,errmsg3,msgEnterAmount,msgInvalidAmount,msgEnterDate, decimalSymbol,groupSymbol,msgConfirmFunding) {
	...
	var sigDate = trim(document.aimEditActivityFormPopin.signatureDate.value);

	if (sigDate.length == 0) {

		alert ("Signature date not entered");

		document.aimEditActivityFormPopin.signatureDate.focus();

		return false;

	}*/

	var numComm = document.aimEditActivityFormPopin.numComm.value;

	var numDisb = document.aimEditActivityFormPopin.numDisb.value;

	var numExp = document.aimEditActivityFormPopin.numExp.value;
...
function setTranscationType(type) {
	...
	if (type == "addCommitments") {
		...
		document.aimEditActivityFormPopin.numComm.value++;
		...
		document.aimEditActivityFormPopin.numExp.value++;
		...
		document.aimEditActivityFormPopin.numDisb.value++;
