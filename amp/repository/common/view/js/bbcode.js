...
function checkForm() {
	...
	if (curForm.description.value.length < 2) {
...
function emoticon(text) {
	text = ' ' + text + ' ';
	if (curForm.description.createTextRange && curForm.description.caretPos) {
		var caretPos = curForm.description.caretPos;
		caretPos.text = caretPos.text.charAt(caretPos.text.length - 1) == ' ' ? text + ' ' : text;
		curForm.description.focus();
	} else {
	curForm.description.value  += text;
	curForm.description.focus();
	}
}
...
function bbfontstyle(bbopen, bbclose) {
	if ((clientVer >= 4) && is_ie && is_win) {
		...
		if (!theSelection) {
			curForm.description.value += bbopen + bbclose;
			curForm.description.focus();
			return;
		}
	...
	storeCaret(curForm.description);
}
...
function bbstyle(bbnumber) {
	...
	if (bbnumber == -1) { // Close all open tags & default button names
		while (bbcode[0]) {
			...
			curForm.description.value += bbtags[butnumber + 1];
	...
	if (donotinsert) {		// Close all open tags up to the one just clicked & default button names
		...
		if (imageTag && (bbnumber != 14)) {		// Close image tag before adding another
			curForm.description.value += bbtags[15];
		...
		// Open tag
		curForm.description.value += bbtags[bbnumber];
