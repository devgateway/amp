...
<!-- BREADCRUMB START -->
<div class="centering">
<span class="sec_name"><digi:trn>Sector Profile Dashboard</digi:trn></span><span class="breadcrump_sep">|</span><a href=# class="l_sm"><digi:trn>Dashboards</digi:trn></a><span class="breadcrump_sep"><b>»</b></span><span class="bread_sel"><digi:trn>Sector Profile Dashboard</digi:trn></span>
</div>
<br/>
<!-- BREADCRUMB END -->
...
<table border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td>
    <div class="dashboard_name" id="sector_name">
    	<c:if test="${visualizationform.filter.sectorId eq '-1' }">
	    	<digi:trn>ALL SECTORS</digi:trn>
    	</c:if>
    	<c:if test="${visualizationform.filter.sectorId ne '-1' }">
			<c:forEach var="sector" items="${visualizationform.filter.sectors}">
...
function refreshBoxes(o){
	...
	for(var j = 0; j < results.children.length; j++){
		...
		var sectorPlaceholder = document.getElementById("sector_name");
		
		var sectorName = document.getElementById("sector_dropdown_id").options[document.getElementById("sector_dropdown_id").selectedIndex].text;
		var subSectorName = document.getElementById("sub_sector_dropdown_id").options[document.getElementById("sub_sector_dropdown_id").selectedIndex].text;
		sectorPlaceholder.innerHTML = sectorName + "<br/><span style=\"font-size:16px\">" + subSectorName + "</span>";
