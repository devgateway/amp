...
public class InterchangeEndpoints {
    ...
    @ApiOperation(
            value = "Get project thumbnail image",
            notes = "This endpoint returns the thumbnail image associated with a specific project/activity.\n\n" +
                    "The thumbnail is returned as a binary image file with the appropriate content type. " +
                    "If no thumbnail is available for the specified project, a default image or a 404 response " +
                    "will be returned depending on the system configuration.\n\n" +
                    "This endpoint is useful for displaying project preview images in listings or dashboards.")
    ...
    @ApiOperation(value = "Get field values by their IDs",
    ...
    @Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
    @ApiMethod(id = "getIdValues", ui = false)
    @ApiOperation(     value = "Get field values by their IDs using a specific FM template",
    ...
    @ApiOperation(value = "Get complete list of activity fields and their configuration",
    ...
    @ApiOperation(
            value = "Get filtered activity information by ID",
            notes = "This endpoint allows you to retrieve specific fields of an activity identified by its ID.\n\n" +
                    "Unlike the GET version of this endpoint, which returns all fields, this POST version allows " +
                    "you to specify exactly which fields you want to retrieve. This is useful for optimizing " +
                    "response size and processing time when you only need certain parts of an activity.\n\n" +
                    "The request body should contain a filter object that specifies which fields to include in the response. " +
                    "The response will contain only the requested fields and their values for the specified activity.\n\n" +
                    "This endpoint requires appropriate view permissions for the requested activity.")
    ...
    @ApiOperation(
            value = "Retrieve multiple activities by their AMP IDs",
            notes = "This endpoint allows you to fetch information for multiple activities in a single request " +
                    "using their AMP IDs (business identifiers) rather than system IDs.\n\n" +
                    "The request body should contain an array of AMP ID strings for which you want to retrieve information.\n\n" +
                    "The response includes complete information for each requested activity. For each valid AMP ID, " +
                    "the response will contain a complete activity object with all available fields. For invalid or " +
                    "inaccessible AMP IDs, the response will include an error object with details about the failure.\n\n" +
                    "This endpoint is particularly useful for integrations that work with business identifiers " +
                    "rather than system IDs, and when you need to retrieve multiple activities efficiently.")
    @ApiResponses(@ApiResponse(code = HttpServletResponse.SC_OK,
    ...
    @ApiOperation( value = "Get activity funding information with currency conversion",
