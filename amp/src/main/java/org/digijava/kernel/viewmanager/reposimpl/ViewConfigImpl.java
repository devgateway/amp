    ...
    public String getFilePath(String moduleName, String moduleInstance,
        ...
        if (moduleName == null) {
            ...
            if ( (moduleLayout != null) &&
                ...
                AmpModule currentAmpModule = (AmpModule) moduleLayout.getModule().get(
                    moduleName);

                if (moduleInstance != null) {
                    // Check, is there any configuration for particular instance
                    if (currentAmpModule.getInstance().containsKey(moduleInstance)) {
                        logger.debug(
                            "This instance is configured let's get file name");
                        // This instance is configured let's get file name
                        Instance instanceConf = (Instance) currentAmpModule.
                            getInstance().
                            get(moduleInstance);
                        if (teaserName == null) {
                            if (pageName == null) {
                    ...
                    if (fileName == null) {
                        if (teaserName == null) {
                            if (pageName == null) {
                                ...
                                // file.
                                fileName = currentAmpModule.getTeaserFile();
                            }
                            else {
                                logger.debug(
                                    "Instance is not configured. we need secondary page");
                                // Instance is not configured. we need secondary page
                                Page page = (Page) currentAmpModule.getPage().get(
                                    pageName);
                                if (page != null) {
                                    fileName = page.getValue();
                                }
                            ...
                            // We need non-default teaser
                            Teaser teaser = (Teaser) currentAmpModule.getTeaser().
                                get(
                                teaserName);
                            if (teaser != null) {
                                if (teaser.isRenderJSP()) {
                    ...
                    // Instance is not set. we need secondary page
                    Page page = (Page) currentAmpModule.getPage().get(pageName);
                    if (page != null) {
                        fileName = page.getValue();
                    }
