...
import org.digijava.kernel.siteconfig.AmpModule;
...
public abstract class ViewConfigUtil
    ...
    protected void expandModulesFromRepository(SiteConfig siteConfig) {
        ...
        while (repositoryIter.hasNext()) {
            ...
            RepositoryLayout components = (RepositoryLayout) entry.getValue();
            String moduleName = (String) entry.getKey();
            AmpModule repositoryAmpModule = components.getModule();

            if (repositoryAmpModule != null) {
                AmpModule siteConfAmpModule = (AmpModule) modules.get(moduleName);
                if (siteConfAmpModule == null) {
                    AmpModule newAmpModule = new AmpModule();
                    newAmpModule.setName(moduleName);
                    newAmpModule.setTeaserFile(repositoryAmpModule.getTeaserFile());

                    Iterator reposModIter = repositoryAmpModule.getTeaser().values().
                        iterator();
                    while (reposModIter.hasNext()) {
                        Teaser item = (Teaser) reposModIter.next();
                        newAmpModule.addTeaser(item);
                    }

                    reposModIter = repositoryAmpModule.getPage().values().iterator();
                    while (reposModIter.hasNext()) {
                        Page item = (Page) reposModIter.next();
                        newAmpModule.addPage(item);
                    }

                    siteConfig.getModuleLayout().addModule(newAmpModule);
                }
                else {
                    siteConfAmpModule.merge(repositoryAmpModule);
                }
            }
