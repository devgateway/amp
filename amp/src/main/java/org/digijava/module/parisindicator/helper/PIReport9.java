...
public class PIReport9 extends PIAbstractReport {
    ...
    private static Logger logger = Logger.getLogger(PIReport9.class);
    private final String reportCode = PIConstants.PARIS_INDICATOR_REPORT_9;
                    ...
                    if (auxAmpAhsurvey.getAmpDonorOrgId().getAmpOrgId().equals(
                        ...
                        while (iterFundingDetails.hasNext()) {
                            ...
                            if (auxFundingDetail.getTransactionType().intValue() == Constants.DISBURSEMENT
                                    && auxFundingDetail.getAdjustmentType().getValue().equals(CategoryConstants.ADJUSTMENT_TYPE_PLANNED.getValueKey())) {

                                auxRow = new PIReport9Row();
                                // Check survey answers for this AmpAhsurvey.
                                boolean[] showColumn = PIUtils.getSurveyAnswers(PIConstants.PARIS_INDICATOR_REPORT_9,
                                        auxAmpAhsurvey);
                                ...
                                toExchangeRate = 0;
                                if (currency != null) {
                                    toExchangeRate = Util.getExchange(currency.getCurrencyCode(), new java.sql.Date(
                                            auxFundingDetail.getTransactionDate().getTime()));
                                }
    ...
    public Collection<PIReportAbstractRow> reportPostProcess(Collection<PIReportAbstractRow> baseReport, int startYear,
        ...
        Comparator compareRows = new Comparator() {
            ...
            public int compare(Object o1, Object o2) {
                PIReport9Row aux1 = (PIReport9Row) o1;
                PIReport9Row aux2 = (PIReport9Row) o2;
        ...
        while (iter.hasNext()) {
            ...
            if (auxGroup.getAmpOrgGrpId().equals(row.getDonorGroup().getAmpOrgGrpId())) {
                ...
                if (row.getYear() == currentYear) {
                    ...
                    PIReport9Row newRow = new PIReport9Row();
    ...
    private Collection<PIReportAbstractRow> calculatePercentages(Collection<PIReportAbstractRow> coll, int startYear,
        ...
        while (iterColl.hasNext()) {
            ...
            PIReport9Row auxRow = (PIReport9Row) iterColl.next();
            if (auxRow.getColumn3().doubleValue() > 0) {
                auxRow.setColumn4(auxRow.getColumn1().add(auxRow.getColumn2()).multiply(new BigDecimal(100)).divide(
                        auxRow.getColumn3(), RoundingMode.HALF_UP).floatValue());
            } else {
                auxRow.setColumn4(0);
            }
        ...
        for (int i = 0; i < endYear + 1 - startYear; i++) {
            PIReport9Row auxRow = new PIReport9Row();
    ...
    private Collection<PIReportAbstractRow> addMissingYears(Collection<PIReportAbstractRow> coll, int startYear,
        ...
        while (iterRet.hasNext()) {
            PIReport9Row rowRet = (PIReport9Row) iterRet.next();
            Iterator iterOrigen = coll.iterator();
            while (iterOrigen.hasNext()) {
                PIReport9Row rowOrigen = (PIReport9Row) iterOrigen.next();
