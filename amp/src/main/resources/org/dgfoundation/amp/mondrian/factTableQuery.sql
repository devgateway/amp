...
INSERT INTO mondrian_fact_table (entity_id, funding_id, entity_internal_id, transaction_type, adjustment_type, transaction_date, date_code, display_date_code, 
  ...
  SELECT 
	...
	rawdonation.transaction_amount * (
     ...
     CASE WHEN src_role='EA' THEN rawdonation.originating_org_id WHEN dest_role='EA' THEN rawdonation.dest_org_id ELSE @@BUGCHOOSER@@ END AS ea_org_id,
     CASE WHEN src_role='BA' THEN rawdonation.originating_org_id WHEN dest_role='BA' THEN rawdonation.dest_org_id ELSE @@BUGCHOOSER@@ END AS ba_org_id,
     CASE WHEN src_role='IA' THEN rawdonation.originating_org_id WHEN dest_role='IA' THEN rawdonation.dest_org_id ELSE @@BUGCHOOSER@@ END AS ia_org_id,
     CASE WHEN src_role='RO' THEN rawdonation.originating_org_id WHEN dest_role='RO' THEN rawdonation.dest_org_id ELSE @@BUGCHOOSER@@ END AS ro_org_id,
     CASE WHEN src_role='CA' THEN rawdonation.originating_org_id WHEN dest_role='CA' THEN rawdonation.dest_org_id ELSE @@BUGCHOOSER@@ END AS ca_org_id,
     CASE WHEN src_role='RG' THEN rawdonation.originating_org_id WHEN dest_role='RG' THEN rawdonation.dest_org_id ELSE @@BUGCHOOSER@@ END AS rg_org_id,
     CASE WHEN src_role='SG' THEN rawdonation.originating_org_id WHEN dest_role='SG' THEN rawdonation.dest_org_id ELSE @@BUGCHOOSER@@ END AS sg_org_id,
