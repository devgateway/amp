...
import { degreesToRadians, useValueFormatter, Container, useDimensions, clampArc, SvgWrapper, ResponsiveWrapper } from '@nivo/core';
import { ArcsLayer, ArcLabelsLayer } from '@nivo/arcs';
...
var useRadialBar = function useRadialBar(_ref) {
  var data = _ref.data,
      ...
      colors = _ref$colors === void 0 ? commonDefaultProps.colors : _ref$colors,
  ...
  var getColor = useOrdinalColorScale(colors, 'category');
  ...
  var valueScale = useMemo(function () {
    return castLinearScale(scaleLinear().domain([0, maxValue]).range([startAngle, endAngle]));
  }, [maxValue, startAngle, endAngle]);
  var radiusScale = useMemo(function () {
    return castBandScale(scaleBand().domain(serieIds).range([innerRadius, outerRadius]).padding(padding));
  ...
  var arcGenerator = useMemo(function () {
    ...
    }).cornerRadius(cornerRadius).padAngle(degreesToRadians(padAngle));
  ...
  var tracks = useMemo(function () {
    return getScaleTicks(radiusScale).map(function (value) {
...
var InnerRadialBar = function InnerRadialBar(_ref) {
  ...
  var _clampArc = clampArc(originalStartAngle, originalEndAngle),
  ...
  if (layers.includes('grid')) {
    layerById.grid = jsxs(Fragment, {
      children: [jsx(PolarGrid, {
        center: center,
        ...
        startAngle: startAngle,
        endAngle: endAngle
      }), radialAxisStart && jsx(RadialAxis, _objectSpread2({
        type: "start",
        center: center,
        angle: Math.min(startAngle, endAngle),
        scale: radiusScale
      }, radialAxisStart)), radialAxisEnd && jsx(RadialAxis, _objectSpread2({
        type: "end",
        center: center,
        angle: Math.max(startAngle, endAngle),
        scale: radiusScale
      }, radialAxisEnd)), circularAxisInner && jsx(CircularAxis, _objectSpread2({
        type: "inner",
        center: center,
        radius: innerRadius,
        startAngle: startAngle,
        endAngle: endAngle,
        scale: valueScale
      }, circularAxisInner)), circularAxisOuter && jsx(CircularAxis, _objectSpread2({
        type: "outer",
        center: center,
        radius: outerRadius,
        startAngle: startAngle,
  ...
  if (layers.includes('tracks') && enableTracks) {
    layerById.tracks = jsx(RadialBarTracks, {
      center: center,
      tracks: tracks,
      arcGenerator: arcGenerator,
      transitionMode: transitionMode
  ...
  if (layers.includes('bars')) {
    layerById.bars = jsx(RadialBarArcs, {
      center: center,
      bars: bars,
      borderWidth: borderWidth,
      borderColor: borderColor,
  ...
  if (layers.includes('labels') && enableLabels) {
    layerById.labels = jsx(ArcLabelsLayer, {
      center: center,
      data: bars,
      label: label,
  ...
  if (layers.includes('legends') && legends.length > 0) {
    layerById.legends = jsx(Fragment, {
...
var RadialBar = function RadialBar(_ref2) {
  ...
  return jsx(Container, {
    ...
    theme: theme,
    children: jsx(InnerRadialBar, _objectSpread2({
      isInteractive: isInteractive
    }, otherProps))
  });
};
  ...
  return jsx(ResponsiveWrapper, {
    children: function children(_ref) {
      var width = _ref.width,
          height = _ref.height;
      return jsx(RadialBar, _objectSpread2({
        width: width,
        height: height
      }, props));
    }
...
export { RadialBar, ResponsiveRadialBar, commonDefaultProps, svgDefaultProps };
