<%@ page pageEncoding="UTF-8" %>
<%@ taglib uri="http://digijava.org/digi" prefix="digi" %>

<table cellpadding="0" cellspacing="0" width="100%" background="/WEB-INF/repository/aim/images/bg-home.jpg" border="0" >
	<tr>
		<td valign="top">
			<table height=95 cellspacing="0" cellpadding="0" width=757 
			background="/WEB-INF/repository/aim/images/bg-home.jpg" valign="top" border="0" class=r-dotted>
				<tr>
					<td>
						<table cellspacing="0" cellpadding="0" width="100%">
							<tr>
								<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>	
								<td class=home-bg-header rowSpan=2 width="100%" valign="top">
										<P>
										<digi:trn key="aim:ampSiteTitle">
											MINISTRY OF FINANCE AND ECONOMIC DEVELOPMENT
										</digi:trn>
										</P> 
								</td>
							</tr>      
							<tr>
							  	<td>&nbsp;</td>
							</tr>
							<tr>
							  	<td>&nbsp;</td>
								<td class=home-bg-text valign="top">
								<digi:trn key="aim:ampHomePageDescriptionLine">
								AMP is used to report and manage aid flows from donors.<br> 
								This site is accessible to registered users only. 
								</digi:trn>
								</td>
							</tr>
						</table>
					</td>
				</tr>
			</table>
		</td>
	</tr>
</table>
