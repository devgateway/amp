...
function addkey()
{
	window.document.form1.action="<%=request.getContextPath()%>/translate/global/GlobalAddKey.do";
...
<body bgcolor="white">
	...
	<input type="hidden" name="src_locale" value="en">
      ...
      <h3>Global translation interface for site <%=DgUtil.getSiteDomain(request).getSite().getName()%></h3>	
			  ...
			  <tr align="right">
				  ...
				  if(request.getParameter("search")!=null)
				  {
					...
					%>
						...
						<a href="global.do?siteId=dgMarket&method=keySearch&startFrom=<%=(pagenum*50)+1%>&search=<%=search%>&radio_locale1=<%=source_locale%>&radio_locale2=<%=target_locale%>&pagenum=<%=pagenum+1%>">Next Page</a>
						</th>
				  	 <%}
					if(pagenum > 1 && rownum < 50)
					{
					%>
						<th width="49%" align="right">
						<a href="global.do?method=keySearch&siteId=dgMarket&startFrom=<%=((pagenum-2)*50)+1%>&search=<%=search%>&radio_locale1=<%=source_locale%>&radio_locale2=<%=target_locale%>&pagenum=<%=pagenum-1%>">Previous Page</a>
				  	...
				  	if(pagenum > 1 && rownum == 50)
						...
						<th width="49%" align="right">
						<a href="global.do?method=keySearch&siteId=dgMarket&startFrom=<%=((pagenum-2)*50)+1%>&search=<%=search%>&radio_locale1=<%=source_locale%>&radio_locale2=<%=target_locale%>&pagenum=<%=pagenum-1%>">Previous Page</a>
						<a href="global.do?method=keySearch&siteId=dgMarket&startFrom=<%=(pagenum*50)+1%>&search=<%=search%>&radio_locale1=<%=source_locale%>&radio_locale2=<%=target_locale%>&pagenum=<%=pagenum+1%>">Next Page</a>
						</th>
				  	<%}
			  ...
			  <select name="search" onChange="document.form1.method.value='keySearch';document.form1.mykey.value='';document.form1.new_text.value='';document.form1.choice.value='';document.form1.multi.value='0';document.form1.buttontype.value='0';document.form1.submit()">
				...
				disabled="true" 
					...
					<logic:equal name="smessage" property="targetSiteId" value='<%=DgUtil.getRootSite(DgUtil.getSiteDomain(request).getSite()).getId().toString()%>'>
						<%if(DgUtil.getRootSite(DgUtil.getSiteDomain(request).getSite()).getId() != DgUtil.getSiteDomain(request).getSite().getId()){%>
							<textarea name="txt<%=rownum1%>" cols="30" rows="3" style="color:blue;background-color: white"><bean:write name="smessage" property="target"/></textarea>
						<%}else{%>
