	...
	public static void main(String[] args) {
		...
		List<ImmutablePair<ReportSpecification, BenchmarkResult>> specs = Arrays.asList(
			new ImmutablePair<>(
					...
					new BenchmarkResult(null, 115, 48, 2920, 
			...
			new ImmutablePair<>(
					ReportSpecificationImpl.buildFor(
						...
						GroupingCriteria.GROUPING_YEARLY),
					new BenchmarkResult(null, 101, 37, 2426, null)
					),
			new ImmutablePair<>(
					ReportSpecificationImpl.buildFor(
						"bySectorByLocation", 
						...
						GroupingCriteria.GROUPING_YEARLY),
					new BenchmarkResult(null, 158, 44, 2426, null)
					),
			new ImmutablePair<>(
					ReportSpecificationImpl.buildFor(
					...
					new BenchmarkResult(null, 223, 53, 2426, null)
					),
			new ImmutablePair<>(
					...
					new BenchmarkResult(null, 116, 15, 1748, null))
			);
		...
		System.out.println("reportName,reportEntities,reportTime,outputTime,reportSpeedup,outputSpeedup");
		for(String reportName:runRes.keySet()) {
			...
			System.out.format("%s,%d,%d,%d,%.2f,%.2f\n", reportName, run.nrEntities, run.runMillies, run.outputMillies, speedup(run.runMillies, cor.runMillies), speedup(run.outputMillies, cor.outputMillies));
		}
	}
