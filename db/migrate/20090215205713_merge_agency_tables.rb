class MergeAgencyTables < ActiveRecord::Migration  
  def self.up
    ...
    contr_link = conn.select_all("SELECT * FROM contracted_agencies_projects")
    
    contr_link.each do |cl|
      agency_name = conn.select_all("SELECT name FROM agencies WHERE id = '#{cl["contracted_agency_id"]}'").first["name"]
      puts "processing contracted link for agency name: #{agency_name}"
      ...
      conn.update("UPDATE contracted_agencies_projects SET contracted_agency_id = '#{corresponding_impl_id}' WHERE contracted_agency_id = '#{cl["contracted_agency_id"]}'")
    end
