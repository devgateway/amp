 ...
 * 	 Created: Sep 16, 2003
 * 	 CVS-ID: $Id: EditSite.java,v 1.1 2005-07-06 10:34:09 rahul Exp $
 *
...
public class EditSite
  ...
  public ActionForward execute(ActionMapping mapping,
    ...
    HashSet newSiteDomains = new HashSet();
    ...
    while (iter.hasNext()) {
      ...
      if ( (tmpSite != null) && (!tmpSite.getId().equals(currentSite.getId()))) {
        Object[] params = {
            item.getDomain(),
      ...
      if (item.getLangCode() != null) {
        Locale lang = (Locale) languages.get(item.
                                             getLangCode());
        siteDomain.setLanguage(lang);
        if (lang != null) {
          siteDomainLangs.add(item.
                              getLangCode());
        }
      }
      else {
    ...
    if (defLanguage == null) {
      ...
      if (parent == null) {
        ...
        iter = SiteCache.getInstance().getUserLanguages(parent).iterator();
        while (iter.hasNext()) {
    ...
    if (errors.isEmpty()) {
      ...
      // If inheritSecurity attribute was changed, DBPolicy must be
      // reloaded
      boolean reloadPolicy = false;
      if (realSite.isInheritSecurity() != siteForm.isInheritSecurity()) {
        realSite.setInheritSecurity(siteForm.isInheritSecurity());
        reloadPolicy = true;
      }
