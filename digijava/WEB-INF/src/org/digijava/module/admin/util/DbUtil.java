...
tx.rollback();
	...
	try {
		...
		if (item.getSite() == null) {
		    ...
		    Object[] params = {
			  item.getModuleInstanceId().toString(),
			  new Integer(GroupPermission.
				      MODULE_INSTANCE_PERMISSION)
		    };
		    Type[] types = {
			  Hibernate.STRING, Hibernate.INTEGER};
		    Iterator permsIter = sess.iterate(" from p in class " +
			  GroupPermission.class.getName() +
			  " where p.targetName= ? and p.permissionType = ?",
			  params, types);

		    while (permsIter.hasNext()) {
			GroupPermission groupPerm = (GroupPermission) permsIter.
			      next();
			Group permGrp = groupPerm.getGroup();
			permGrp.getPermissions().remove(groupPerm);
			sess.delete(groupPerm);
			permsIter.remove();
			sess.update(permGrp);
		    }
...
tx.rollback();
	...
	try {
	    ...
	    Locale oldLocale = (Locale) session.load(Locale.class,
		  locale.getCode());
	    oldLocale.setAvailable(locale.isAvailable());

	    session.update(oldLocale);
