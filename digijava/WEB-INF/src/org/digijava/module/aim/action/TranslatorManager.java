		...
		if(request.getParameter("import")!=null)
		{
	        ...
	        try {
				...
				if (trns_in.getTrn() != null) {
					...
					//deleting all translations for all the languages from imported file
					/*
					{
						it=languagesImport.iterator();
						while(it.hasNext())
						{
							String langImported=(String)it.next();
							deleteTransForLang(langImported);
						}
					}
					//insert all languages from imported file*/
					/*{
							int i=0;System.out.println("aaaaaa");
							it = trns_in.getTrn().iterator();
						while(it.hasNext())
						{
							Trn element = (Trn) it.next();
							...
							//updateNewTranslationMessage(msg,element.getLangIso());
							updateNewTranslationMessage(msg,"fr");
							i++;
						}
