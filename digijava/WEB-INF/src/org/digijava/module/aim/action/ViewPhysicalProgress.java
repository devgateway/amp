...
public class ViewPhysicalProgress extends TilesAction
{
							   ...
							   throws IOException,ServletException 
	{
		...
		AmpActivity ampActivity = DbUtil.getProjectChannelOverview(ampActivityId) ;
		...
		if ( ampActivity != null )
		{
			...
			dbReturnSet1 = DbUtil.getAmpComponent( ampActivity.getAmpActivityId() ) ;
			...
			while ( iter1.hasNext() )
			{
				...
				if (progress.getCurrency() != null) {
					sc.setCurrency(progress.getCurrency().getCurrencyCode());
				}
				date = DateConversion.ConvertDateToString(progress.getReportingDate()) ;
				...
				while (iter2.hasNext()) {
					...
					AmpPhysicalPerformance phypm = (AmpPhysicalPerformance) iter2.next();
					physicalProgress.setPid(phypm.getAmpPpId()) ;
					physicalProgress.setTitle(phypm.getTitle()) ;
					date = DateConversion.ConvertDateToString(phypm.getReportingDate()) ;
