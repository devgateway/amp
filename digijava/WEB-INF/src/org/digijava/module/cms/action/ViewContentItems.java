/*
 *   ViewContentItems.java
 *   @Author Maka Kharalashvili maka@digijava.org
 * 	 Created: May 06,2004
 * 	 CVS-ID: $Id: ViewContentItems.java,v 1.1 2005-07-06 10:34:09 rahul Exp $
 *
...
public class ViewContentItems
  ...
  public ActionForward execute(ActionMapping mapping,
    ...
    tabsList.add(all);
    CmsTabItem published = new CmsTabItem("Published", CMSContentItemForm.VIEW_MODE_PUBLISHED);
    tabsList.add(published);
    CmsTabItem pending = new CmsTabItem("pending", CMSContentItemForm.VIEW_MODE_PENDING);
    tabsList.add(pending);
    CmsTabItem rejected = new CmsTabItem("rejected", CMSContentItemForm.VIEW_MODE_REJECTED);
    ...
    if (contItemForm.getViewMode() == null) {
      ...
      all.setActive(true);
        ...
        all.setActive(true);
    } else if (contItemForm.getViewMode().equals(CMSContentItemForm.VIEW_MODE_PUBLISHED)) {
        published.setActive(true);
    } else if (contItemForm.getViewMode().equals(CMSContentItemForm.VIEW_MODE_PENDING)) {
      pending.setActive(true);
    } else if (contItemForm.getViewMode().equals(CMSContentItemForm.VIEW_MODE_REJECTED)) {
    ...
    //dbItemsList = DbUtil.getContentItems(new Long(cms.getId()));
    dbItemsList = DbUtil.getContentItems(cms.getId(), contItemForm.getViewMode(),
                                         getOffset(), contItemForm.getItemsPerPage() + 1);
    ...
    if (dbItemsList != null && dbItemsList.size() > 0) {
      ...
      for( int i = 0; i < n; i++ ) {
        ...
        CMSContentItemForm.CMSContentItemInfo ci = new CMSContentItemForm.
            CMSContentItemInfo();
        ...
        ci.setAuthorUser(item.getAuthorUser());
        ci.setPublished(item.isPublished());
        ci.setRejected(item.isRejected());
    ...
    return mapping.findForward("viewContentItems");
  }
