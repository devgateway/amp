...
function restart() {
	var field = document.aimCurrencyForm.selectedDate.value;
	if (field == 1) {
		document.aimCurrencyForm.exchangeRateDate.value = '' + padout(curr_day) + '/' + padout(curr_month - 0 + 1) + '/' + curr_year;
	}
	calWindow.close();
}

function newWindow(field) {
	document.aimCurrencyForm.selectedDate.value = field;
