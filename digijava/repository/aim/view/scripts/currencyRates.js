...
function restart() {
	var field = document.aimCurrencyRateForm.selectedDate.value;
	if (field == 1) {
		document.aimCurrencyRateForm.filterByDateFrom.value = '' + padout(curr_day) + '/' + padout(curr_month - 0 + 1) + '/' + curr_year;
	} else if (field == 2) {
		document.aimCurrencyRateForm.filterByDateTo.value = '' + padout(curr_day) + '/' + padout(curr_month - 0 + 1) + '/' + curr_year;	
	} else if (field == 3) {
		document.aimCurrencyRateForm.updateCRateDate.value = '' + padout(curr_day) + '/' + padout(curr_month - 0 + 1) + '/' + curr_year;	
	}
	calWindow.close();
}

function newWindow(field) {
	document.aimCurrencyRateForm.selectedDate.value = field;
