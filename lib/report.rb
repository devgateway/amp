...
module Report
  ...
  def self.query_options    
    [['donor', ll(:terms, :donor), true], ['agency', ll(:reports, :donor_agency), true], ['donor_project_number', ll(:reports, :project_number), true], 
    ['oecd_number', ll(:reports, :oecd_id)], ['recipient_country_budget_nr', ll(:reports, :budget_number)], ['title', ll(:reports, :project_title), true], 
    ['description', ll(:reports, :project_desc)], ['strategy_link', ll(:reports, :cs_link)], ['prj_status', ll(:reports, :project_status)],
    ['start', ll(:reports, :start), true], ['end', ll(:reports, :end), true], 
    ['total_commitments', ll(:reports, :commitments_td), true], ['total_payments', ll(:reports, :disbursements_td), true]] +
    ...
    ['sectors', ll(:terms, :dac_sector)], ['aid_modality', ll(:reports, :toa)], ['grant_loan', "#{ll(:options, :grant)}/#{ll(:options, :loan)}"], 
