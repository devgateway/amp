...
class Funding < ActiveRecord::Base
  ...
  class << self
    ...
    def total_commitments_on_budget
      find(:all,
        :select => 'commitments AS com, currency, year',
        :joins => 'JOIN projects ON project_id = projects.id',
        :conditions => ['projects.data_status = ? and fundings.on_budget = true', Project::PUBLISHED]
      ).inject(0) { |sum, c| c.com.to_currency(c.currency, c.year).in(Prefs.default_currency) + sum }
    end

    def total_commitments_off_budget
      find(:all,
        :select => 'commitments AS com, currency, year',
        :joins => 'JOIN projects ON project_id = projects.id',
        :conditions => ['projects.data_status = ? and fundings.on_budget = false', Project::PUBLISHED]
      ).inject(0) { |sum, c| c.com.to_currency(c.currency, c.year).in(Prefs.default_currency) + sum }
    end
