class ChangeDataTypeOfSectorRelevancesAmount < ActiveRecord::Migration
  def self.up
    execute "DROP VIEW sector_payments"
    execute "DROP VIEW project_dac_sector_shares"
    change_column :sector_relevances, :amount, :float
    execute <<-END_SQL
  ...
  def self.down
    execute "DROP VIEW sector_payments"
    execute "DROP VIEW project_dac_sector_shares"
    change_column :sector_relevances, :amount, :integer
    execute <<-END_SQL
      CREATE VIEW project_dac_sector_shares AS
      	SELECT SUM(sector_relevances.amount) AS amount, projects.id AS project_id, sector_relevances.dac_sector_id
      	FROM projects, sector_relevances
