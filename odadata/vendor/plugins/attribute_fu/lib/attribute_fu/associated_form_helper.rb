module AttributeFu
  ...
  module AssociatedFormHelper
    ...
    def add_associated_link(name, object, opts = {})
      ...
      fields_for_opts  = (opts.delete(:fields_for) || {}).merge({:javascript => true})
      
      form_builder     = self # because the value of self changes in the block
      
      function = "if (typeof #{variable} == 'undefined') #{variable} = 0; 
                    $(#{container}).append($.template("+[self.render_associated_form(object, :fields_for => fields_for_opts, :partial => partial)].flatten.first.to_json+"), { number: --#{variable}});"
