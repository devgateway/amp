...
select ac.activity_id as amp_activity_id,COALESCE( concat((select category_value from amp_category_value where id=substr(cp.value,1,locate(' ', cp.value)) ),' ',substr(cp.value,locate(' ', cp.value)) )  , substr(cp.value,locate(' ', cp.value))) 
