...
SELECT m.message_key as mkey, m.lang_iso as miso, m.site_id msid, 'test',now() FROM dg_message m 
group by lower(m.message_key), m.lang_iso, m.site_id;         

update dg_message_dup mt, dg_message m 
set mt.message_utf8=m.message_utf8, mt.created=m.created 
where mt.message_key=m.message_key and mt.lang_iso=m.lang_iso and mt.site_id=m.site_id;    



update dg_message_dup m set m.message_key=lower(message_key);
