...
<digi:form action="/showTaxonomyManager.do">
<html:hidden name="cmsForm" property="parentCategoryId" value="0"/>
<html:hidden name="cmsForm" property="categoryId" value="0"/>
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
			<tr>
				...
				<td width="99%" class="mainHeader" align="center" valign="middle">
					<digi:trn key="cms:taxonomyManager">Taxonomy manager</digi:trn>
				</td>
					...
					<td>
						<table width="100%" height="1" cellpadding="0" cellspacing="0" border="0">
							<tr>
								...
								<td class="rowItem"><digi:img src="module/cms/images/tree/spacer.gif" width="10" border="0"/></td>
								<td class="rowItem" width="50%" valign="middle" nowrap align="left">
									<input type="button" value="Add" onClick="javascript:addCategory()" class="rowItemButton">
									<input name="toolbarItem" type="button" value="Edit" onClick="javascript:editCategory()" class="rowItemButton" disabled>
									<input name="toolbarItem" type="button" value="Delete" class="rowItemButton" onClick="deleteCategory()" disabled>
								</td>
...
<script language="JavaScript">
	...
	function addCategory() {
	  <digi:context name="addCategory" property="context/module/moduleinstance/addCategory.do" />
      document.cmsForm.action = "<%= addCategory %>";
      document.cmsForm.submit();
	}
	
	function editCategory() {
	  <digi:context name="editCategory" property="context/module/moduleinstance/editCategory.do" />
      document.cmsForm.action = "<%= editCategory %>";
      document.cmsForm.submit();
	}	
	
	function deleteCategory() {
	  <digi:context name="deleteCategory" property="context/module/moduleinstance/deleteCategory.do" />
      document.cmsForm.action = "<%= deleteCategory %>";
      document.cmsForm.submit();
	}	

	function userSelect (selNode){
		if (selNode.getNamedAttribute("id") != null) {
			document.getElementsByName("categoryId")[0].value = selNode.getNamedAttribute("id").value;
			enableToolbarItems (true);
		} else {
			document.getElementsByName("categoryId")[0].value = 0;
			enableToolbarItems (false);
		}
		if (selNode.getNamedAttribute("parId") != null) {
			document.getElementsByName("parentCategoryId")[0].value = selNode.getNamedAttribute("parId").value;
		} else {
