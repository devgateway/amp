...
<%@ taglib uri="/taglib/digijava" prefix="digi" %>
	...
	<div class="subtabs">
		...
		<button id="step${stepNum}_next_button" type="button" class="toolbar-dis" 
			<c:if test="${stepNum==3 || stepNum==0}">disabled="disabled"</c:if>
			onclick="navigateTab(1)">
		...
		<button type="button" class="toolbar-dis" onclick="importActivities()" name="loadFile" id="de_btn_import_${stepNum}" 
			<c:if test="${stepNum!=0}">disabled="disabled" </c:if>
		>
