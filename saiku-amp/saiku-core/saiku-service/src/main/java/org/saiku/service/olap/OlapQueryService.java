...
public class OlapQueryService implements Serializable {
    ...
    public CellDataSet execute(String queryName, String formatter, int start,
			int end) {
        formatter = formatter == null ? "" : formatter.toLowerCase();
        ICellSetFormatter formatterObject = new CellSetFormatter();
        if(formatter.equals("flat")) {
        	formatterObject = new CellSetFormatter();
        }
        else if (formatter.equals("hierarchical")) {
        	formatterObject = new HierarchicalCellSetFormatter();
        }
        else if (formatter.equals("flattened")) {
        	formatterObject = new FlattenedCellSetFormatter();
        }
        
        formatterObject.setStart(start);
        formatterObject.setEnd(end);

        return execute(queryName, formatterObject);
	}
    //End Custom DG Code
